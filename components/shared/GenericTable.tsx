        ...
        <div className="border-t border-border bg-card md:bg-muted/10 px-3 sm:px-4 py-1.5 flex items-center justify-between gap-2 shrink-0">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground min-w-0">
            ...
            <span className="hidden sm:inline tabular-nums">
              <span className="font-semibold text-foreground">{totalRecords}</span> bản ghi
              <span className="text-muted-foreground/60"> · {Math.min((page - 1) * pageSize + 1, totalRecords)}–{Math.min(page * pageSize, totalRecords)}</span>
            </span>
            {/* Mobile: compact range */}
            <span className="sm:hidden tabular-nums text-muted-foreground">
              ...
              <span className="text-muted-foreground/50"> / {totalRecords}</span>
            </span>
            ...
            {selectedIds.size > 0 && (
              <>
                ...
                <span className="sm:hidden inline-flex items-center justify-center h-5 min-w-[20px] px-1 rounded-full bg-primary/15 text-primary text-2xs font-bold tabular-nums">
                  {selectedIds.size}✓
                </span>
              </>
            )}

            {/* Desktop: page size select */}
            <div className="hidden sm:flex items-center gap-1.5 border-l border-border pl-2">
              <select
                value={pageSize}
                onChange={(e) => onPageSizeChange(Number(e.target.value))}
                className="h-6 w-14 border border-border rounded bg-card px-1 text-xs font-medium text-foreground outline-none focus:ring-1 focus:ring-primary/20 transition-all cursor-pointer"
              >
                ...
                {totalRecords > 100 && <option value={totalRecords}>Tất cả</option>}
              </select>
              <span className="text-muted-foreground/60 text-2xs">/ trang</span>
            </div>
            {/* Mobile: page size select (compact, same options) */}
            <div className="sm:hidden flex items-center border-l border-border pl-2">
              <select
                value={pageSize}
                onChange={(e) => onPageSizeChange(Number(e.target.value))}
                className="h-7 min-w-[3rem] border border-border rounded bg-card px-1.5 text-xs font-medium text-foreground outline-none focus:ring-1 focus:ring-primary/20 cursor-pointer"
