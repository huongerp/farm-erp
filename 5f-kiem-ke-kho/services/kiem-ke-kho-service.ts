...
const repoDot = createRepository<DotKiemKeKho>({
  tableName: 'kho_van_dot_kiem_ke_kho',
  mockData: JSON.parse(JSON.stringify(seedDot)),
  delay: 300,
});

const seedChiTiet: ChiTietKiemKeKho[] = [
  {
    id: 'ctkk-1',
...
const repoChiTiet = createRepository<ChiTietKiemKeKho>({
  tableName: 'kho_van_chi_tiet_kiem_ke_kho',
  mockData: JSON.parse(JSON.stringify(seedChiTiet)),
  delay: 200,
});
...
export async function getDotKiemKeKhoById(id: string): Promise<DotKiemKeKho | null> {
  await delay(300);
  const found = await repoDot.getById(id);
...
export async function getChiTietByDot(id_dot_kiem_ke_kho: string): Promise<ChiTietKiemKeKho[]> {
  await delay(300);
  const all = await repoChiTiet.getAll();
  return all.filter((c) => c.id_dot_kiem_ke_kho === id_dot_kiem_ke_kho);
}
...
export async function createDotKiemKeKho(data: DotKiemKeKhoCreate): Promise<DotKiemKeKho> {
  ...
  return repoDot.insert(dot as DotKiemKeKho);
}

export async function updateDotKiemKeKho(id: string, data: Partial<DotKiemKeKhoCreate>): Promise<DotKiemKeKho> {
  await delay(400);
  const current = await repoDot.getById(id);
  if (!current) throw new Error('Đợt kiểm kê không tồn tại');
  ...
  const updated = await repoDot.update(id, {
...
export async function deleteDotKiemKeKho(ids: string[]): Promise<void> {
  await delay(400);
  const all = await repoDot.getAll();
  const onlyDraft = all.filter((d) => ids.includes(d.id) && d.trang_thai === 'draft');
  if (onlyDraft.length !== ids.length) {
    throw new Error('Chỉ được xóa đợt ở trạng thái Nháp');
  }
  const allCt = await repoChiTiet.getAll();
  const ctToRemove = allCt.filter((c) => ids.includes(c.id_dot_kiem_ke_kho)).map((c) => c.id);
  if (ctToRemove.length) await repoChiTiet.remove(ctToRemove);
  await repoDot.remove(ids);
}
...
export async function changeTrangThaiDot(id: string, trang_thai: TrangThaiDotKiemKeKho): Promise<DotKiemKeKho> {
  ...
  const current = await repoDot.getById(id);
  if (!current) throw new Error('Đợt kiểm kê không tồn tại');
  const updated = await repoDot.update(id, { trang_thai, tg_cap_nhat: now() });
  const [enriched] = await enrichDot([updated]);
  return enriched;
}
...
export async function taoDanhSachKiemKe(
  ...
  const existingKeys = new Set(
    allCt.filter((c) => c.id_dot_kiem_ke_kho === id_dot_kiem_ke_kho).map((c) => `${c.id_kho}|${c.id_hang_hoa}`)
  );

  const toRemove = allCt.filter((c) => c.id_dot_kiem_ke_kho === id_dot_kiem_ke_kho).map((c) => c.id);
  if (toRemove.length) await repoChiTiet.remove(toRemove);
  ...
  for (const id_kho of dot.id_kho) {
    ...
    for (const { id_hang_hoa, so_luong } of tonTheoKho) {
      ...
      await repoChiTiet.insert({
        ...
        tg_tao: now(),
        tg_cap_nhat: now(),
      } as Omit<ChiTietKiemKeKho, 'id'> & { id?: string });
      existingKeys.add(key);
    }
  }

  await repoDot.update(id_dot_kiem_ke_kho, { trang_thai: 'dang_kiem_ke', tg_cap_nhat: now() });
...
export async function updateChiTietKetQua(
  ...
  const updated = await repoChiTiet.update(id_chi_tiet, {
    so_luong_thuc_te,
    ghi_chu_dong,
    id_nguoi_kiem,
    ...
    ngay_kiem: now(),
    tg_cap_nhat: now(),
    ket_qua: computeKetQua({ ...current, so_luong_thuc_te: so_luong_thuc_te ?? undefined, ghi_chu_dong: ghi_chu_dong ?? undefined }),
  });
  return updated;
}
...
export async function dieuChinhTonTheoKetQua(id_chi_tiet: string): Promise<void> {
  await delay(300);
  const c = await repoChiTiet.getById(id_chi_tiet);
  ...
  await capNhatTonKho(c.id_kho, c.id_hang_hoa, bien_dong);
}
...
export async function hoanThanhDot(id_dot_kiem_ke_kho: string): Promise<DotKiemKeKho> {
  ...
  const updated = await repoDot.update(id_dot_kiem_ke_kho, { trang_thai: 'hoan_thanh', tg_cap_nhat: now() });
  const [enriched] = await enrichDot([updated]);
  return enriched;
}
