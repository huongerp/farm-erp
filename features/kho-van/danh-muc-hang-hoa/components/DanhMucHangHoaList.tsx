...
const treeOptions = {
  getId: (d: DanhMucHangHoa) => d.id,
  getParentId: (d: DanhMucHangHoa) => d.id_cha,
  getOrder: (d: DanhMucHangHoa) => d.thu_tu,
...
const DanhMucHangHoaList: React.FC<Props> = ({
  data,
  columns,
  selectedIds,
  onToggleSelection,
  ...
  if (isLoading) {
    return (
      <ListPageSkeleton
        loadingText={t('danhMucHangHoa.loading')}
  ...
  if (data.length === 0) {
    return (
      <div className="flex-1 min-h-0 flex items-center justify-center p-4">
        <EmptyState
          title={t('danhMucHangHoa.empty')}
          description={t('danhMucHangHoa.emptyHint')}
  ...
  const renderCell = (item: DanhMucHangHoa, col: ColumnConfig) => {
    ...
    <div className="flex flex-col h-full min-h-0 bg-card overflow-hidden">
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center justify-end gap-0 flex-nowrap">
                          {isParent && onAddChild && (
                            <Tooltip content={t('danhMucHangHoa.detail.addChild')} placement="top">
                              <button
                                type="button"
      ...
      <div className="shrink-0 border-t border-border bg-muted/30">
        <TablePaginationFooter
          ...
          recordsLabel={t('danhMucHangHoa.footerRecords')}
        />
      </div>
    </div>
  );
...
export default DanhMucHangHoaList;
