...
const DanhSachNhaCungCapList: React.FC<Props> = ({
  data,
  columns,
  selectedIds,
  onToggleSelection,
  ...
  if (isLoading) {
    return (
      <ListPageSkeleton
        loadingText={t('nhaCungCap.loading')}
  ...
  if (data.length === 0) {
    return (
      <div className="flex-1 min-h-0 flex items-center justify-center p-4">
        <EmptyState
          title={t('nhaCungCap.empty')}
          description={t('nhaCungCap.emptyHint')}
    ...
    <div className="flex flex-col h-full min-h-0 bg-card overflow-hidden">
      ...
      <div className="shrink-0 border-t border-border bg-muted/30">
        <TablePaginationFooter
          ...
          recordsLabel={t('nhaCungCap.footerRecords')}
        />
      </div>
    </div>
  );
...
export default DanhSachNhaCungCapList;
