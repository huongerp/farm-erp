...
import { exportPhieuKiemKeKhoToPDF } from './utils/export-phieu-kiem-ke-kho';
...
const PhieuKiemKeKhoPreviewPage: React.FC = () => {
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    setExporting(true);
    try {
      await exportPhieuKiemKeKhoToPDF(dot, chiTiet);
    } finally {
      setExporting(false);
    }
  }, [dot, chiTiet]);
  ...
  return (
    <>
      ...
      >
        <div className="phieu-kiem-ke-kho-preview-toolbar flex items-center justify-between gap-3 px-4 py-3 bg-card border-b border-border shadow-sm shrink-0">
          <button
            type="button"
            onClick={handleClose}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Download size={16} />
              {t('kiemKeKho.preview.download')} PDF
            </button>
            <button
              type="button"
              onClick={handlePrint}
