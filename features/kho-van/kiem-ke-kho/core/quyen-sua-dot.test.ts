...
const HANH_DONG = [
  ['sửa đợt', coTheSuaDot],
  ['xoá đợt', coTheXoaDot],
  ['sửa chi tiết', coTheSuaChiTiet],
  ['chuyển trạng thái', coTheChuyenTrangThaiDot],
] as const;
...
describe('quyền đụng vào đợt kiểm kê', () => {
  ...
  it('chuyển trạng thái xét trạng thái HIỆN TẠI, không xét đích đến', () => {
    ...
    expect(coTheChuyenTrangThaiDot('hoan_thanh', false)).toBe(false);
    // Nhưng vẫn được chốt sổ một đợt đang kiểm kê.
    expect(coTheChuyenTrangThaiDot('dang_kiem_ke', false)).toBe(true);
  });
});
