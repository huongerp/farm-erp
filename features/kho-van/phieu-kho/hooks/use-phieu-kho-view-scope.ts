 ...
 * Phiếu kho: lọc theo kho_id và kho_den_id. Nhập/xuất: theo kho_id. Chuyển kho: xem nếu thuộc chi nhánh của kho_id hoặc kho_den_id (OR).
 */
...
export interface PhieuKhoViewScope {
...
export function usePhieuKhoViewScope(): PhieuKhoViewScope {
  ...
  return useMemo(() => {
    const defaultScope: PhieuKhoViewScope = {
      viewAll: true,
      viewByBranch: false,
      allowedBranchIds: [],
