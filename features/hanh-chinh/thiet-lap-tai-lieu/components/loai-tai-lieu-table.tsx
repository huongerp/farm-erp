...
interface Props {
  ...
  onRowClick?: (item: LoaiTaiLieu) => void;
}

const LoaiTaiLieuTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onRowClick }) => {
  ...
  const renderCell = (colId: string, item: LoaiTaiLieu) => {
    switch (colId) {
      case 'ma':
        return (
  ...
  const renderMobileCard = (item: LoaiTaiLieu, isSelected: boolean) => (
    <div
      role="button"
      tabIndex={0}
...
export default LoaiTaiLieuTable;
