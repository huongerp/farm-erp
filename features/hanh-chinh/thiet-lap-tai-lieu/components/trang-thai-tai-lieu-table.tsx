...
interface Props {
  ...
  onRowClick?: (item: TrangThaiTaiLieu) => void;
}

const TrangThaiTaiLieuTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onRowClick }) => {
  ...
  const renderCell = (colId: string, item: TrangThaiTaiLieu) => {
    switch (colId) {
      case 'ma':
        return (
  ...
  const renderMobileCard = (item: TrangThaiTaiLieu, isSelected: boolean) => (
    <div
      role="button"
      tabIndex={0}
...
export default TrangThaiTaiLieuTable;
