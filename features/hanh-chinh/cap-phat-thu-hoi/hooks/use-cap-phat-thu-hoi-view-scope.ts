 ...
 * - quan_tri (admin/all) hoặc nhân viên cấp bậc 1 → viewAll: hiển thị đủ tab Lịch sử + Của tôi + Thống kê.
...
export interface CapPhatThuHoiViewScope {
  /** true: được xem tab Lịch sử (quan_tri hoặc cấp bậc 1). false: chỉ xem Của tôi + Thống kê. */
  viewAll: boolean;
  isLoading: boolean;
}
