...
const StatsCharts: React.FC<Props> = ({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
      {chartByType.some((d) => d.value > 0) && (
        <div className="bg-card rounded-xl border border-border p-3.5">
          ...
          <ResponsiveContainer width="100%" height={STATS_CHART_HEIGHT}>
            <PieChart>
              <Pie
                data={chartByType.filter((d) => d.value > 0)}
                cx="50%"
                cy="50%"
                outerRadius={80}
                innerRadius={40}
                ...
                stroke="none"
              >
                {chartByType.filter((d) => d.value > 0).map((_, i) => (
      ...
      {chartByNguoiThucHien.length > 0 && (
        <div className="bg-card rounded-xl border border-border p-3.5 lg:col-span-2">
          <div className="flex items-center gap-2 mb-3">
            <User size={14} className="text-primary" />
            <h3 className="text-xs font-semibold text-foreground">
              {t('capPhatThuHoi.stats.byPerformer')}
            </h3>
          </div>
          <ResponsiveContainer width="100%" height={STATS_CHART_HEIGHT}>
            <BarChart
              data={chartByNguoiThucHien}
              barSize={28}
              margin={{ top: 4, right: 4, left: 4, bottom: 4 }}
            >
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" opacity={0.5} />
