/**
 * Trang preview phiếu lương (mở tab mới) – chỉ nội dung + toolbar In / Tải, không sidebar.
 * Route: /phieu-luong/:id
 */
...
const PayslipPreviewPage: React.FC = () => {
  ...
  const periodStr = record ? `${record.nam}-${String(record.thang).padStart(2, '0')}` : '';
  const titleLabel = record ? (record.ten_nhan_vien || record.ma_nhan_vien || record.id) : '';

  useEffect(() => {
    if (!record) return;
    const prev = document.title;
    document.title = titleLabel && periodStr
      ? `${t('bangLuong.preview.title')} - ${titleLabel} · ${periodStr}`
      : t('bangLuong.preview.title');
    return () => { document.title = prev; };
  }, [record, titleLabel, periodStr, t]);
  ...
  const handleClose = useCallback(() => {
    if (window.history.length > 1) {
      ...
      navigate('/hanh-chinh/bang-luong', { replace: true });
    }
  }, [navigate]);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-muted/30 p-4">
        <p className="text-destructive font-medium text-center">
          {loadError ? (error?.message ?? t('bangLuong.preview.loadError')) : t('bangLuong.preview.notFound')}
        </p>
