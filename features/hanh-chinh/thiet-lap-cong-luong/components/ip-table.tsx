...
interface Props {
  ...
  onView?: (item: PayrollWifiIp) => void;
}

const PayrollWifiIpTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onView }) => {
      ...
      case 'ip_wifi':
        return (
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-sm text-foreground">{item.ip_wifi}</span>
          </div>
        );
      case 'ghi_chu':
        return (
  ...
  const renderMobileCard = (item: PayrollWifiIp, isSelected: boolean) => (
    <div
      role="button"
      tabIndex={0}
      onClick={() => onView?.(item)}
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            <span className="text-xs font-mono text-muted-foreground">{item.ip_wifi}</span>
      ...
      <div className="grid grid-cols-1 gap-2 px-3 py-2 bg-muted/30 rounded-lg mb-3 text-body-sm">
        <div>
          <p className="text-muted-foreground mb-0.5">{t('payrollIp.store.noteCol')}</p>
...
export default PayrollWifiIpTable;
