...
interface Props {
  ...
  onView?: (item: AssetStorageLocation) => void;
}

const NoiLuuTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onView }) => {
  ...
  const renderCell = (colId: string, item: AssetStorageLocation) => {
    switch (colId) {
      case 'ten_chi_nhanh':
        return (
          <div className="flex flex-col gap-0.5 min-w-[160px]">
            <span className="font-medium text-foreground text-sm">{item.ten_chi_nhanh || '—'}</span>
            <span className="text-xs text-muted-foreground font-mono">{item.id_chi_nhanh}</span>
          </div>
        );
      case 'ma_noi_luu':
        return (
          <span className="font-mono text-sm font-medium text-foreground">{item.ma_noi_luu}</span>
        );
      case 'ten_noi_luu':
        return (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-foreground">{item.ten_noi_luu}</span>
          </div>
        );
      case 'ghi_chu':
        return (
  ...
  const renderMobileCard = (item: AssetStorageLocation, isSelected: boolean) => (
    <div
      role="button"
      tabIndex={0}
      onClick={() => onView?.(item)}
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <h4 className="font-semibold text-foreground text-sm truncate">{item.ten_noi_luu}</h4>
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            <span className="text-xs font-mono text-muted-foreground">{item.ma_noi_luu}</span>
            <span className="text-xs text-muted-foreground">{item.ten_chi_nhanh || '—'}</span>
      ...
      {item.ghi_chu ? (
        <div className="px-3 py-2 bg-muted/30 rounded-lg mb-3 text-body-sm">
          <p className="text-muted-foreground mb-0.5">{t('thietLapTaiSan.noiLuu.store.noteCol')}</p>
