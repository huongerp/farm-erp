...
const TrangThaiDetail: React.FC<Props> = ({ data, onClose, onEdit, onDelete }) => {
  const { t } = useTranslation();
  const { data: taiSanList = [] } = useTaiSanList();
  const assetsWithStatus = useMemo(
    () => taiSanList.filter((ts) => ts.id_trang_thai === data.id),
    [taiSanList, data.id]
  );
  const viewAllHref = `/hanh-chinh/danh-muc-tai-san?id_trang_thai=${encodeURIComponent(data.id)}`;
  ...
  return (
    ...
    >
      <div className="space-y-5">
        <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-body-sm text-muted-foreground font-mono mt-0.5">
              {data.ma} · {t('thietLapTaiSan.trangThai.store.thuTuCol')} {data.thu_tu}
            </p>
        ...
        <DetailSection
          title={t('thietLapTaiSan.trangThai.form.basicInfo')}
          icon={<Tag size={14} />}
          variant="primary"
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <DetailField label={t('thietLapTaiSan.trangThai.store.maCol')} value={data.ma} />
            <DetailField label={t('thietLapTaiSan.trangThai.store.tenCol')} value={data.ten} />
            <DetailField label={t('thietLapTaiSan.trangThai.store.thuTuCol')} value={String(data.thu_tu)} />
            {data.ghi_chu ? (
              <div className="col-span-1 sm:col-span-2">
                <DetailField label={t('thietLapTaiSan.trangThai.store.noteCol')} value={data.ghi_chu} />
              </div>
            ) : null}
            <DetailField
              label={t('thietLapTaiSan.trangThai.store.statusCol')}
              value={isActive ? t('common.active') : t('common.inactive')}
            />
            <DetailField
              label={t('thietLapTaiSan.trangThai.store.updatedCol')}
        ...
        <TaiSanListSection
          title={t('danhSachTaiSan.detail.assetsWithStatus')}
