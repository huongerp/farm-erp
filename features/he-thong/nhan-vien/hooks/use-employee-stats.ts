...
export function useEmployeeStats({
  ...
  const filtered = useMemo(() => {
    ...
    return employees.filter((emp) => {
      ...
      const matchStatus =
        filterStatus.length === 0 || filterStatus.includes(String(emp.trang_thai));
      return matchDept && matchStatus;
    });
  }, [employees, filterDept, filterStatus, dateRange.end]);

  const total = filtered.length;
  const active = filtered.filter((e) => e.trang_thai === TRANG_THAI_NV.DANG_LAM_VIEC).length;
  ...
  const trends = useMemo((): StatsTrends => {
    ...
    const filteredLastYear = employees.filter((e) => {
      ...
      const matchStatus = filterStatus.length === 0 || filterStatus.includes(String(e.trang_thai));
      return matchDept && matchStatus;
    });
    const totalLastYear = filteredLastYear.length;
    const activeLastYear = filteredLastYear.filter((e) => e.trang_thai === TRANG_THAI_NV.DANG_LAM_VIEC).length;
  ...
  const deptSummary = useMemo(() => {
    ...
    filtered.forEach((e) => {
      ...
      map[dept].total++;
      if (e.trang_thai === TRANG_THAI_NV.DANG_LAM_VIEC) map[dept].active++;
      else if (e.trang_thai === TRANG_THAI_NV.THU_VIEC) map[dept].probation++;
