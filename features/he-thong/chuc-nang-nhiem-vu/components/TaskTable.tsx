...
import { Edit, Trash2, Power, ClipboardList } from 'lucide-react';
import GenericTable from '../../../../components/shared/GenericTable';
import { useTaskTableStore } from '../store/useFunctionTaskStore';
...
interface Props {
  ...
  onStatusChange: (item: Task) => void;
}

const TaskTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onStatusChange }) => {
  ...
  const renderCell = (colId: string, item: Task) => {
    switch (colId) {
      case 'ten_chuc_nang':
        return (
          <span className="text-sm font-medium text-muted-foreground">{item.ten_chuc_nang || '--'}</span>
        );
      case 'ma_nhiem_vu':
        return (
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-sm">{item.ma_nhiem_vu}</span>
          </div>
        );
      case 'ten_nhiem_vu':
        return <span className="font-medium text-foreground">{item.ten_nhiem_vu}</span>;
      case 'mo_ta':
        return (
  ...
  const renderMobileCard = (item: Task, isSelected: boolean) => (
    <div
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-semibold text-foreground">{item.ten_nhiem_vu}</p>
            <p className="text-xs text-muted-foreground font-mono">{item.ma_nhiem_vu}</p>
          </div>
        </div>
        <input
          type="checkbox"
      ...
      <p className="text-xs text-muted-foreground mb-2">{item.ten_chuc_nang}</p>
