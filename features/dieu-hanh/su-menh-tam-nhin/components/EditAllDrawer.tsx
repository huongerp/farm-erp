...
import { Compass, Heart, Plus, Trash2 } from 'lucide-react';
...
import { editAllFormSchema, type EditAllFormValues } from '../core/schema';
import type { CoreValue } from '../core/types';
import { useUpdateMissionVision, useUpdateValues } from '../hooks/use-su-menh-tam-nhin';
...
function BehaviorList({
  ...
  control: Control<EditAllFormValues>;
  index: number;
  name: 'hanh_vi_nen_lam' | 'hanh_vi_khong_nen_lam';
  label: string;
}) {
  const { t } = useTranslation();
  const fieldName = `gia_tri.${index}.${name}` as FieldArrayPath<EditAllFormValues>;
...
const EditAllDrawer: React.FC<Props> = ({
  initialMission,
  initialVision,
  values,
  onClose,
  ...
  const defaultValues: EditAllFormValues = {
    su_menh: initialMission ?? '',
    tam_nhin: initialVision ?? '',
    gia_tri: initialGiaTri,
  };

  const { register, handleSubmit, control, reset, formState: { errors } } = useForm<EditAllFormValues>({
    resolver: zodResolver(editAllFormSchema),
    defaultValues,
  });
  ...
  useEffect(() => {
    const next =
      values.length > 0
        ? values.sort((a, b) => a.thu_tu - b.thu_tu).map(mapValueToForm)
        : [{ ten: '', mo_ta: '', thu_tu: 0, mo_dich: '', hanh_vi_nen_lam: [], hanh_vi_khong_nen_lam: [] }];
    reset({ su_menh: initialMission ?? '', tam_nhin: initialVision ?? '', gia_tri: next });
  }, [initialMission, initialVision, values, reset]);

  const onSubmit: SubmitHandler<EditAllFormValues> = async (data) => {
    await updateMissionVision.mutateAsync({ su_menh: data.su_menh, tam_nhin: data.tam_nhin });
    ...
    await updateValues.mutateAsync({ gia_tri: normalized });
    toast.success(t('suMenhTamNhin.saveSuccess'));
    onClose();
  };

  const isLoading = updateMissionVision.isPending || updateValues.isPending;

  return (
    <GenericDrawer
      title={t('suMenhTamNhin.editAllTitle')}
      icon={<Compass size={20} />}
      onClose={onClose}
      footer={
        <FormDrawerFooter
          formId="edit-all-form"
          onCancel={onClose}
          isLoading={isLoading}
          isEdit
    ...
    >
      <form id="edit-all-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
