...
import { useDonViTinhStore } from '../store/useDonViTinhStore';
...
interface Props {
  ...
  onEdit: (item: DonViTinh) => void;
  onDelete: (id: string) => void;
}

const ThietLapDonViTinhList: React.FC<Props> = ({
  data,
  isLoading,
  page,
  pageSize,
  ...
  const { columns } = useDonViTinhStore();
  ...
  const renderCell = (item: DonViTinh, col: ColumnConfig) => {
    switch (col.id) {
      case 'thu_tu':
        return (
      ...
      case 'ky_hieu':
        return (
          <td key={col.id} className="px-4 py-3" style={getColumnCellStyle(col)}>
            <span className="text-muted-foreground">{item.ky_hieu || '—'}</span>
          </td>
        );
      default:
  ...
  if (isLoading) {
    return (
      <ListPageSkeleton
        loadingText={t('tieuChiKpi.thietLapDvt.searchPlaceholder')}
            ...
            <tbody className="divide-y divide-border [&>tr:last-child>td]:border-b [&>tr:last-child>td]:border-border">
              {isEmpty ? (
                <tr>
                  ...
                  >
                    <EmptyState
                      title={t('tieuChiKpi.thietLapDvt.empty')}
                      icon={<Ruler className="w-10 h-10 text-muted-foreground mx-auto" />}
                    />
                  </td>
                </tr>
              ) : (
...
export default ThietLapDonViTinhList;
