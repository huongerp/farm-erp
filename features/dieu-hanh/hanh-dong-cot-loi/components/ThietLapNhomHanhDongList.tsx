...
interface Props {
  ...
  onEdit: (item: NhomHanhDong) => void;
  onDelete: (id: string) => void;
}

const ThietLapNhomHanhDongList: React.FC<Props> = ({
  data,
  isLoading,
  page,
  pageSize,
  ...
  const { columns } = useNhomHanhDongStore();
  ...
  const renderCell = (item: NhomHanhDong, col: ColumnConfig) => {
    switch (col.id) {
      case 'thu_tu':
        return (
  ...
  if (isLoading) {
    return (
      <ListPageSkeleton
        loadingText={t('hanhDongCotLoi.thietLap.searchPlaceholder')}
            ...
            <tbody className="divide-y divide-border [&>tr:last-child>td]:border-b [&>tr:last-child>td]:border-border">
              {isEmpty ? (
                <tr>
                  ...
                  >
                    <EmptyState
                      title={t('hanhDongCotLoi.thietLap.empty')}
                      icon={<Layers className="w-10 h-10 text-muted-foreground mx-auto" />}
                    />
                  </td>
                </tr>
              ) : (
...
export default ThietLapNhomHanhDongList;
