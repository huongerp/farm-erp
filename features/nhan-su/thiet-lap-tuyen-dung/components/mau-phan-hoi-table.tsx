...
interface Props {
  ...
  onView?: (item: MauPhanHoi) => void;
}

const MauPhanHoiTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onView }) => {
  ...
  const renderCell = (colId: string, item: MauPhanHoi) => {
    switch (colId) {
      case 'ma':
        return (
      ...
      case 'ten_loai':
        return (
          <span className="text-sm text-foreground">{item.ten_loai}</span>
        );
      case 'tieu_de':
        return (
  ...
  const renderMobileCard = (item: MauPhanHoi, isSelected: boolean) => (
    <div
      role="button"
      tabIndex={0}
      onClick={() => onView?.(item)}
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <h4 className="font-semibold text-foreground text-sm truncate">{item.ten_loai}</h4>
      ...
      {item.tieu_de ? (
        <div className="grid grid-cols-1 gap-2 px-3 py-2 bg-muted/30 rounded-lg mb-3 text-body-sm">
          <p className="text-muted-foreground mb-0.5">{t('thietLapTuyenDung.mauPhanHoi.store.tieuDeCol')}</p>
...
export default MauPhanHoiTable;
