...
interface Props {
  ...
  onView?: (item: TrangThaiUngVien) => void;
}

const TrangThaiUngVienTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onView }) => {
  ...
  const renderCell = (colId: string, item: TrangThaiUngVien) => {
    switch (colId) {
      case 'ma':
        return (
      ...
      case 'loai_ket_qua':
        if (!item.loai_ket_qua) return <span className="text-xs text-muted-foreground">—</span>;
        return item.loai_ket_qua === 'onboard' ? (
  ...
  const renderMobileCard = (item: TrangThaiUngVien, isSelected: boolean) => (
    <div
      role="button"
      tabIndex={0}
      onClick={() => onView?.(item)}
    ...
    >
      ...
      {item.ghi_chu ? (
        <div className="grid grid-cols-1 gap-2 px-3 py-2 bg-muted/30 rounded-lg mb-3 text-body-sm">
          <p className="text-muted-foreground mb-0.5">{t('thietLapTuyenDung.trangThaiUngVien.store.noteCol')}</p>
...
export default TrangThaiUngVienTable;
