...
interface Props {
  ...
  onView?: (item: KenhTuyenDung) => void;
}

const KenhTuyenDungTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onView }) => {
  ...
  const renderCell = (colId: string, item: KenhTuyenDung) => {
    switch (colId) {
      case 'ma':
        return (
  ...
  const renderMobileCard = (item: KenhTuyenDung, isSelected: boolean) => (
    <div
      role="button"
      tabIndex={0}
      onClick={() => onView?.(item)}
    ...
    >
      ...
      {item.ghi_chu ? (
        <div className="grid grid-cols-1 gap-2 px-3 py-2 bg-muted/30 rounded-lg mb-3 text-body-sm">
          <p className="text-muted-foreground mb-0.5">{t('thietLapTuyenDung.kenhTuyenDung.store.noteCol')}</p>
...
export default KenhTuyenDungTable;
