...
const StatsToolbar: React.FC<StatsToolbarProps> = ({
  ...
  const actions = (
    <div className="relative shrink-0" ref={actionsRef}>
      ...
      >
        <span>{t('baoCaoTuyenDung.actions')}</span>
      ...
      {actionsOpen && (
        <div className="absolute right-0 top-full mt-1.5 z-50 min-w-[160px] bg-card rounded-xl shadow-xl border border-border overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
          <button
            ...
            disabled={!canExport}
            className="w-full h-9 px-3 flex items-center gap-2 text-left text-sm text-foreground hover:bg-muted/60 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
