...
import {
  exportPhieuKiemKePTToPDF,
  exportPhieuKiemKePTToDoc,
...
const EXPORT_OPTIONS: { format: PhieuKiemKePTExportFormat; label: string; icon: React.ReactNode }[] = [
...
const PhieuKiemKePTPreviewPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    document.title = `${t('kiemKeKhoPT.preview.title')} - ${dot.ma_dot} · ${dot.ten_dot}`;
    return () => {
      document.title = prev;
    };
  }, [dot, t]);
  ...
  const handlePrint = () => {
    ...
    // (L:20mm R:15mm T:15mm B:15mm được đặt trong PhieuKiemKePTPreviewContent)
  ...
  const handleExport = useCallback(async (format: PhieuKiemKePTExportFormat) => {
    if (!dot) return;
    setDownloadOpen(false);
    setExporting(true);
    try {
      if (format === 'pdf') await exportPhieuKiemKePTToPDF(dot, chiTiet);
      else if (format === 'doc') await exportPhieuKiemKePTToDoc(dot, chiTiet);
      else if (format === 'xlsx') await exportPhieuKiemKePTToXLSX(dot, chiTiet);
    } catch (e) {
  ...
  return (
    <>
      <div
        ...
        role="main"
        aria-label={t('kiemKeKhoPT.preview.title')}
      >
        <div className="phieu-kiem-ke-kho-preview-toolbar flex items-center justify-between gap-3 px-4 py-3 bg-card border-b border-border shadow-sm shrink-0 print:hidden">
          ...
          <div className="flex items-center gap-2">
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <Download size={16} />
                {t('kiemKeKhoPT.preview.download')}
          ...
          <div className="bg-white shadow-xl rounded-sm phieu-kiem-ke-kho-preview-content-wrapper print:shadow-none" style={{ width: '210mm', minHeight: '297mm' }}>
            {chiTietLoading && !chiTiet.length ? (
              <div className="flex items-center justify-center p-12 text-muted-foreground">
                {t('kiemKeKhoPT.loading')}
              </div>
            ) : (
              <PhieuKiemKePTPreviewContent dot={dot} chiTiet={chiTiet} />
            )}
          </div>
        </div>
      </div>
