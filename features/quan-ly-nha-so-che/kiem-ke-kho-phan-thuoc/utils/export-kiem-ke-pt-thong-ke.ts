 ...
 *
 * Input: kết quả từ useKiemKeKhoPTStats + meta bộ lọc đang áp dụng.
 */
import type { KiemKeKhoPTStatsSummary, KiemKeKhoPTStatsByTrangThai } from '../components/stats/useKiemKeKhoPTStats';
...
export interface KiemKeKhoPTThongKeExportMeta {
  dateFrom?: string;
  dateTo?: string;
...
function buildThongKeBodyHTML(
  summary: KiemKeKhoPTStatsSummary,
  byTrangThai: KiemKeKhoPTStatsByTrangThai[],
  meta: KiemKeKhoPTThongKeExportMeta
): string {
  ...
  if (meta.dateFrom || meta.dateTo) {
    const parts: string[] = [];
    if (meta.dateFrom) parts.push(`${t('kiemKeKhoPT.filter.dateFrom')}: ${meta.dateFrom}`);
    if (meta.dateTo) parts.push(`${t('kiemKeKhoPT.filter.dateTo')}: ${meta.dateTo}`);
  ...
  const kpiCards = [
    { label: t('kiemKeKhoPT.stats.total'), value: summary.total },
    { label: t('kiemKeKhoPT.trangThaiDot.draft'), value: summary.draft },
    { label: t('kiemKeKhoPT.trangThaiDot.dang_kiem_ke'), value: summary.dangKiemKe },
    { label: t('kiemKeKhoPT.trangThaiDot.hoan_thanh'), value: summary.hoanThanh },
  ];
  ...
  const statusTableRows = byTrangThai
    .map(
      (row) =>
        `<tr>
          <td style="${td}">${escapeHtml(t(`kiemKeKhoPT.${row.ten}`))}</td>
...
<div style="font-family:${FONT};font-size:10pt;color:#222;padding:15mm 15mm 15mm 20mm;background:#fff;min-width:500px;box-sizing:border-box">
  ...
  <h1 style="font-size:15pt;text-align:center;margin:0 0 6px;text-transform:uppercase;font-family:${FONT}">${escapeHtml(t('kiemKeKhoPT.stats.title'))}</h1>
  ...
  <h2 style="font-size:11pt;font-weight:600;margin:0 0 8px;font-family:${FONT}">${escapeHtml(t('kiemKeKhoPT.stats.byStatus'))}</h2>
  <table style="width:100%;border-collapse:collapse;font-family:${FONT}">
    <thead>
      <tr>
        <th style="${th}">${escapeHtml(t('kiemKeKhoPT.stats.nameCol'))}</th>
        <th style="${thR}">${escapeHtml(t('kiemKeKhoPT.stats.countCol'))}</th>
      </tr>
    </thead>
    <tbody>${statusTableRows}</tbody>
  </table>

  <p style="font-size:7pt;color:#888;margin-top:20px;border-top:1px solid #e5e7eb;padding-top:8px;font-family:${FONT}">${escapeHtml(t('kiemKeKhoPT.preview.printedAt'))} ${escapeHtml(printedAt)}</p>
</div>`;
}
...
export async function exportKiemKeKhoPTThongKeToPDF(
  summary: KiemKeKhoPTStatsSummary,
  byTrangThai: KiemKeKhoPTStatsByTrangThai[],
  meta: KiemKeKhoPTThongKeExportMeta = {}
): Promise<void> {
...
export async function exportKiemKeKhoPTThongKeToXLSX(
  summary: KiemKeKhoPTStatsSummary,
  byTrangThai: KiemKeKhoPTStatsByTrangThai[],
  meta: KiemKeKhoPTThongKeExportMeta = {}
): Promise<void> {
  ...
  rows.push(
    [],
    [t('kiemKeKhoPT.stats.total'), summary.total],
    [t('kiemKeKhoPT.trangThaiDot.draft'), summary.draft],
    [t('kiemKeKhoPT.trangThaiDot.dang_kiem_ke'), summary.dangKiemKe],
    [t('kiemKeKhoPT.trangThaiDot.hoan_thanh'), summary.hoanThanh],
    [],
    [t('kiemKeKhoPT.stats.nameCol'), t('kiemKeKhoPT.stats.countCol')]
  );

  byTrangThai.forEach((row) => {
    rows.push([t(`kiemKeKhoPT.${row.ten}`), row.count]);
  });

  rows.push([], [t('kiemKeKhoPT.preview.printedAt'), formatDateTime(new Date())]);
