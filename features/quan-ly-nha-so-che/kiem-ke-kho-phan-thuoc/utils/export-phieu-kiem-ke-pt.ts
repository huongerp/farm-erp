 ...
 *
 * Dùng chung builders từ kkpt-preview-layout.ts để nội dung khớp với React preview.
 */
...
function getFileName(dot: DotKiemKePT): string {
...
function buildPhieuKiemKeKhoBodyHTML(dot: DotKiemKePT, chiTiet: ChiTietKiemKePT[], font = FONT): string {
  const t = i18n.t.bind(i18n);
  const title = t('kiemKeKhoPT.preview.title');
  const printedAt = formatDateTime(new Date());
  const subtitle = `${escapeHtml(dot.ma_dot)} · ${escapeHtml(dot.ten_dot)} · ${escapeHtml(getTrangThaiDotLabelPT(dot.trang_thai, t))}`;
  ...
  const detailSectionHtml =
    chiTiet.length > 0
      ? `<h2 style="font-size:11pt;margin:16px 0 8px;font-family:${font}">${escapeHtml(t('kiemKeKhoPT.chiTietSection'))}</h2>${buildKiemKeKhoDetailTableHTML(chiTiet, t, font)}`
      : `<p style="font-size:10pt;color:#666;font-style:italic;font-family:${font}">${escapeHtml(t('kiemKeKhoPT.chiTietEmpty'))}</p>`;
...
export async function exportPhieuKiemKePTToPDF(
  dot: DotKiemKePT,
...
function buildDocHTML(dot: DotKiemKePT, chiTiet: ChiTietKiemKePT[]): string {
...
export async function exportPhieuKiemKePTToDoc(
  dot: DotKiemKePT,
  chiTiet: ChiTietKiemKePT[]
): Promise<void> {
  download(
