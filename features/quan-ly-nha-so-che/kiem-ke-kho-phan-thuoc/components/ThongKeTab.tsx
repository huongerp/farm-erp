...
import { useDotKiemKePTTomTat } from '../hooks/use-kiem-ke-pt';
import {
  exportKiemKeKhoPTThongKeToPDF,
  exportKiemKeKhoPTThongKeToXLSX,
...
function useStatsFilterCounts(items: DotKiemKePTTomTat[]) {
  return useMemo(() => {
...
const ThongKeTab: React.FC = () => {
  ...
  /** Phạm vi xem áp ở server — cùng nguồn suy luận với tab Đợt kiểm kê. */
  ...
  const { data: viewableList = [], isLoading, isError } = useDotKiemKePTTomTat(phamVi);
  ...
  const filteredList = useMemo(() => {
    return viewableList.filter((d: DotKiemKePTTomTat) => {
  ...
  const stats = useKiemKeKhoPTStats(filteredList);
  ...
  const buildExportMeta = () => {
    ...
    if (filterTrangThai.length > 0) {
      filterLabels.push(
        `${t('kiemKeKhoPT.store.trangThaiCol')}: ${filterTrangThai.map((v) => t(`kiemKeKhoPT.trangThaiDot.${v}`)).join(', ')}`
      );
    }
  ...
  const handleExportReport = async () => {
    ...
    try {
      ...
      await exportKiemKeKhoPTThongKeToXLSX(stats.summary, stats.byTrangThai, meta);
    } catch {
  ...
  const handleExportPDF = async () => {
    const meta = buildExportMeta();
    try {
      await exportKiemKeKhoPTThongKeToPDF(stats.summary, stats.byTrangThai, meta);
    } catch {
  ...
  if (isError) {
    return (
      <div className="flex-1 flex items-center justify-center p-4">
        <p className="text-sm text-destructive">{t('kiemKeKhoPT.stats.loadError')}</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-border bg-card shadow-sm overflow-hidden">
        <div className="shrink-0 py-3 px-3 sm:px-4 border-b border-border/50 bg-muted/20">
          <LoadingSpinnerWithText text={t('kiemKeKhoPT.stats.loading')} centered />
        </div>
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-border bg-card shadow-sm overflow-hidden">
      ...
      <div className="kiem-ke-kho-pt-stats-content flex-1 min-h-0 overflow-y-auto custom-scrollbar print:overflow-visible">
        <div className="p-3 sm:p-4 pb-4 space-y-4 print:p-0 print:space-y-3">
          {/* Tiêu đề chỉ hiển thị khi in */}
          <div className="hidden print:block mb-3">
            <p className="text-sm font-semibold text-foreground">{t('kiemKeKhoPT.stats.title')}</p>
            {(dateFrom || dateTo) && (
              <p className="text-xs text-muted-foreground">
                {dateFrom && `${t('kiemKeKhoPT.filter.dateFrom')}: ${dateFrom}`}
                {dateFrom && dateTo && ' — '}
                {dateTo && `${t('kiemKeKhoPT.filter.dateTo')}: ${dateTo}`}
              </p>
            )}
          </div>
          ...
          {isEmpty ? (
            <EmptyState
              title={t('kiemKeKhoPT.stats.noData')}
              description={
                activeFilterCount > 0
                  ? (t('kiemKeKhoPT.stats.noDataHint') || 'Thử xóa bộ lọc.')
                  : t('kiemKeKhoPT.stats.noDataHint')
              }
              action={
                activeFilterCount > 0 ? (
                  <button
            ...
            <>
              <h3 className="text-sm font-semibold text-primary print:hidden">{t('kiemKeKhoPT.stats.title')}</h3>
