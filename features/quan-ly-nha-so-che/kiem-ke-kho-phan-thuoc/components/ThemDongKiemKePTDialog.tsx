...
const ThemDongKiemKePTDialog: React.FC<Props> = ({
  open,
  onClose,
  onConfirm,
  isLoading = false,
  ...
  // fp_farm_danh_sach_hang_hoa không còn cột trang_thai → không lọc theo trạng thái.
  const hangHoaOptions = useMemo(
    () =>
      hangHoaList
        .filter((h) => resolvedKhoIds.some((kId) => !existingKeys.has(`${kId}|${h.id}`)))
        .map((h) => ({
          value: h.id,
          label: `${h.ten_hang_hoa} (${h.ma_hang_hoa})`,
        })),
      ...
      <div className="fixed inset-0 z-[61] flex items-center justify-center p-4 pointer-events-none">
        ...
        >
          ...
          <div className="p-5 space-y-4">
            <Select
              label={t('kiemKeKhoPT.store.hangHoaCol')}
              value={id_hang_hoa}
              onChange={(e) => setIdHangHoa(e.target.value)}
              options={[
                { value: '', label: t('kiemKeKhoPT.themDong.hangHoaPlaceholder') },
                ...hangHoaOptions,
              ]}
            />
            {!isSingleKho && (
              <Select
                ...
                options={[
                  { value: ALL_KHO_VALUE, label: t('kiemKeKhoPT.themDong.tatCaKho') },
                  ...khoOptions,
                ]}
              />
            )}
            {canSubmit && targetKhoIds.length > 1 && (
              <p className="text-xs text-muted-foreground">
                {t('kiemKeKhoPT.themDong.multiKhoHint', { count: targetKhoIds.length })}
              </p>
            )}
          </div>
...
export default ThemDongKiemKePTDialog;
