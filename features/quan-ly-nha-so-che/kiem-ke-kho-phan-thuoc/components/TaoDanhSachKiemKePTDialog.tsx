...
import { getAllFarmDanhMuc } from '../../hang-hoa-phan-thuoc/services/farm-danh-muc-service';
...
interface Props {
  open: boolean;
  onClose: () => void;
  /** (id_dot, filters) — luôn truyền id đợt khi xác nhận để tránh state cũ. */
  onConfirm: (id_dot: string, filters: TaoDanhSachKiemKePTFilters | undefined) => void;
  isLoading?: boolean;
...
const TaoDanhSachKiemKePTDialog: React.FC<Props> = ({
  open,
  onClose,
  onConfirm,
  isLoading = false,
  ...
  // Hàng hóa / danh mục farm không có cột trang_thai → không lọc theo trạng thái.
  const hangHoaOptions = hangHoaList.map((h) => ({
    label: h.ten_hang_hoa,
    value: h.id,
    subLabel: h.ma_hang_hoa,
  }));
  const danhMucOptions = danhMucList.map((d) => ({ label: d.ten_danh_muc, value: d.id }));

  const handleConfirm = useCallback(() => {
    if (!dotId) return;
    const hasAny = id_kho.length > 0 || id_danh_muc.length > 0 || id_hang_hoa.length > 0;
    const filters: TaoDanhSachKiemKePTFilters | undefined = hasAny
      ? {
      ...
      <div className="fixed inset-0 z-[61] flex items-center justify-center p-4 pointer-events-none">
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-foreground">
                  {t('kiemKeKhoPT.taoDanhSachDialog.title')}
                </h3>
                <p className="text-xs text-muted-foreground">
                  {t('kiemKeKhoPT.taoDanhSachDialog.hint')}
                </p>
              </div>
            </div>
            <button
          ...
          <div className="flex-1 min-h-[320px] overflow-visible p-5 space-y-5">
            {khoOptions.length > 0 && (
              <MultiSelect
                label={t('kiemKeKhoPT.taoDanhSachDialog.kho')}
                options={khoOptions}
                value={id_kho}
                onChange={setIdKho}
                placeholder={t('kiemKeKhoPT.taoDanhSachDialog.khoPlaceholder')}
              />
            )}
            <MultiSelect
              label={t('kiemKeKhoPT.taoDanhSachDialog.danhMuc')}
              options={danhMucOptions}
              value={id_danh_muc}
              onChange={setIdDanhMuc}
              placeholder={t('kiemKeKhoPT.taoDanhSachDialog.danhMucPlaceholder')}
            />
            <MultiSelect
              label={t('kiemKeKhoPT.taoDanhSachDialog.hangHoa')}
              options={hangHoaOptions}
              value={id_hang_hoa}
              onChange={setIdHangHoa}
              placeholder={t('kiemKeKhoPT.taoDanhSachDialog.hangHoaPlaceholder')}
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3 px-5 py-4 border-t border-border shrink-0">
            ...
            >
              {t('kiemKeKhoPT.taoDanhSachDialog.confirm')}
            </Button>
          </div>
        </motion.div>
      </div>
...
export default TaoDanhSachKiemKePTDialog;
