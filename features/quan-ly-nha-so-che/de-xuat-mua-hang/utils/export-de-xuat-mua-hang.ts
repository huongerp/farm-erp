...
function fileName(p: DeXuatMuaHang): string {
...
export function buildDeXuatMuaHangBodyHTML(
  phieu: DeXuatMuaHang,
...
<td style="padding:4px 8px;border:1px solid #ccc;font-family:${F};font-size:9pt">${safe(c.ghi_chu)}</td>
      ...
      .join('');
    tableHTML = `
<h2 style="font-size:11pt;margin:12px 0 8px;font-family:${F}">${t('deXuatMuaHang.preview.danhSachChiTiet')}</h2>
<table style="width:100%;border-collapse:collapse;font-family:${F};font-size:10pt">
<thead><tr>${ths}</tr></thead><tbody>${rows}</tbody>
</table>`;
  } else {
    tableHTML = `<p style="font-size:10pt;color:#666;font-style:italic;font-family:${F}">${t('deXuatMuaHang.form.noItems')}</p>`;
  }

  const signBlock = (label: string) =>
    `<div style="text-align:center;flex:1"><p style="font-size:10pt;font-weight:600;color:#333;margin-bottom:2px;font-family:${F}">${label}</p><p style="font-size:8pt;color:#666;font-family:${F}">${t('deXuatMuaHang.preview.signHint')}</p></div>`;
...
<p style="font-size:10pt;color:#555;text-align:center;margin-bottom:12px">(${t('deXuatMuaHang.form.code')}: ${phieu.so_phieu})</p>
<div style="font-size:10pt;margin-bottom:12px">
  <p style="margin:2px 0"><strong style="color:#444">${t('deXuatMuaHang.preview.donVi')}:</strong> ${safe(phieu.ten_noi_de_xuat)}</p>
  <p style="margin:2px 0"><strong style="color:#444">${t('deXuatMuaHang.preview.nguoiTao')}:</strong> ${safe(phieu.ten_nguoi_de_xuat)} . ${t('deXuatMuaHang.preview.boPhan')}: –</p>
  <p style="margin:2px 0"><strong style="color:#444">${t('deXuatMuaHang.preview.ngayLap')}</strong> ${formatDate(phieu.ngay)} (${t('deXuatMuaHang.form.requiredDate')}: ${formatDate(phieu.ngay_can)})</p>
  <p style="margin:2px 0"><strong style="color:#444">${t('deXuatMuaHang.form.notes')}</strong> ${safe(phieu.ghi_chu)}</p>
</div>
${tableHTML}
<div style="display:flex;gap:16px;margin-top:32px;padding-top:16px;border-top:1px solid #ccc">
  ${signBlock(t('deXuatMuaHang.preview.signCreator'))}
  ${signBlock(t('deXuatMuaHang.preview.signChecker'))}
  ${signBlock(t('deXuatMuaHang.preview.signRelated'))}
  ${signBlock(t('deXuatMuaHang.preview.signApprover'))}
</div>
<footer style="margin-top:auto;padding-top:16px;border-top:1px solid #ddd"><p style="font-size:7pt;color:#888;margin:0">${t('deXuatMuaHang.preview.printedAt')} ${printedAt}</p></footer>
</div>`;
}
...
function buildDocHTML(phieu: DeXuatMuaHang, chiTiet: DeXuatMuaHangChiTiet[]): string {
  const t = i18n.t.bind(i18n);
  const info = useUIStore.getState().companyInfo;
  const printedAt = formatDateTime(new Date());
  ...
  const sign = (label: string) =>
    `<td width="25%" style="text-align:center;padding:8px;vertical-align:top"><b>${label}</b><br/><span style="font-size:9pt;color:#666">${t('deXuatMuaHang.preview.signHint')}</span></td>`;
...
<tr><td style="text-align:center;padding:8px 0"><b style="font-size:14pt">ĐỀ XUẤT VẬT TƯ</b><br/>(${t('deXuatMuaHang.form.code')}: ${phieu.so_phieu})</td></tr>
<tr><td style="padding:4px 0"><b>${t('deXuatMuaHang.preview.donVi')}:</b> ${safe(phieu.ten_noi_de_xuat)}</td></tr>
<tr><td style="padding:4px 0"><b>${t('deXuatMuaHang.preview.nguoiTao')}:</b> ${safe(phieu.ten_nguoi_de_xuat)} . ${t('deXuatMuaHang.preview.boPhan')}: –</td></tr>
<tr><td style="padding:4px 0"><b>${t('deXuatMuaHang.preview.ngayLap')}</b> ${formatDate(phieu.ngay)} (${t('deXuatMuaHang.form.requiredDate')}: ${formatDate(phieu.ngay_can)})</td></tr>
<tr><td style="padding:4px 0 12px 0"><b>${t('deXuatMuaHang.form.notes')}:</b> ${safe(phieu.ghi_chu)}</td></tr>
${detailRows ? `
<tr><td style="padding:8px 0 4px 0"><b>${t('deXuatMuaHang.preview.danhSachChiTiet')}</b></td></tr>
...
<tr><td style="padding-top:24px;border-top:1px solid #ccc">
  <table width="100%"><tr>${sign(t('deXuatMuaHang.preview.signCreator'))}${sign(t('deXuatMuaHang.preview.signChecker'))}${sign(t('deXuatMuaHang.preview.signRelated'))}${sign(t('deXuatMuaHang.preview.signApprover'))}</tr></table>
</td></tr>
<tr><td style="padding-top:12px;border-top:1px solid #ddd;font-size:8pt;color:#888">${t('deXuatMuaHang.preview.printedAt')} ${printedAt}</td></tr>
</table>`;
}
...
/**
 * Xuất PDF từ source: HTML build từ buildDeXuatMuaHangBodyHTML → iframe → html2canvas → jsPDF.
 * Phân trang A4: scale theo chiều ngang, cắt ảnh thành nhiều trang.
 */
export async function exportDeXuatMuaHangToPDF(
  phieu: DeXuatMuaHang,
  chiTiet: DeXuatMuaHangChiTiet[],
): Promise<void> {
  ...
  const bodyContent = buildDeXuatMuaHangBodyHTML(phieu, chiTiet);
  const fullHtml = [
...
export async function exportDeXuatMuaHangToDoc(
  phieu: DeXuatMuaHang,
  chiTiet: DeXuatMuaHangChiTiet[],
): Promise<void> {
