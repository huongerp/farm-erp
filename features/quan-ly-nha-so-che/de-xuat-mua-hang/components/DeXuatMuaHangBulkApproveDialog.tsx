...
import ApproveOption, { APPROVE_OPTION_STYLE } from '../../../kho-van/phieu-kho/components/ApproveOption';
import {
  TRANG_THAI_DA_DUYET,
...
interface Props {
  ...
  isPending: boolean;
  onClose: () => void;
  onConfirm: (trangThai: TrangThaiDeXuatMuaHang, ghiChu: string) => void;
}

const TARGET_STATUSES: TrangThaiDeXuatMuaHang[] = [
  TRANG_THAI_DA_DUYET,
  TRANG_THAI_DOI_DUYET,
  TRANG_THAI_KHONG_DUYET,
];
 ...
 */
const DeXuatMuaHangBulkApproveDialog: React.FC<Props> = ({
  count,
  skipped,
  isPending,
  onClose,
  ...
  const [trangThai, setTrangThai] = useState<TrangThaiDeXuatMuaHang>(TRANG_THAI_DA_DUYET);
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-lg font-semibold text-foreground">
                  {t('deXuatMuaHang.bulkApproveDialog.title')}
                </h3>
                <p className="text-sm text-muted-foreground">
                  {t('deXuatMuaHang.bulkApproveDialog.subtitle', { count })}
                </p>
              </div>
            </div>
            <button
          ...
          <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
            {skipped > 0 && (
              <div className="flex gap-2.5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-sm text-amber-800 dark:text-amber-200">
                ...
                <p>{t('deXuatMuaHang.bulkApproveDialog.skipped', { count: skipped })}</p>
              </div>
            )}

            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">
                {t('deXuatMuaHang.bulkApproveDialog.status')}
              </p>
              <div className="grid grid-cols-3 gap-2">
                {TARGET_STATUSES.map((status) => (
                  <ApproveOption
                    key={status}
                    label={t(`deXuatMuaHang.status.${trangThaiToI18nKey(status)}`)}
            ...
            <Textarea
              label={t('deXuatMuaHang.bulkApproveDialog.note')}
              placeholder={t('deXuatMuaHang.bulkApproveDialog.notePlaceholder')}
              value={ghiChu}
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button type="submit" className="bg-primary text-white" disabled={isPending || count === 0}>
                {isPending
                  ? t('deXuatMuaHang.bulkApproveDialog.submitting')
                  : t('deXuatMuaHang.bulkApproveDialog.submit')}
              </Button>
            </div>
          </form>
        </motion.div>
...
export default DeXuatMuaHangBulkApproveDialog;
