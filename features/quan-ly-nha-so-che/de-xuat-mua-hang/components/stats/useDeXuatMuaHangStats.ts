...
export interface DeXuatMuaHangStatsSummary {
  total: number;
  pending: number;
  waiting: number;
  approved: number;
...
export interface DeXuatMuaHangStatsByTrangThai {
  id: string;
  ten: string;
  count: number;
}
...
export function computeDeXuatMuaHangStats(list: DeXuatMuaHang[]) {
    ...
    const byTrangThai: DeXuatMuaHangStatsByTrangThai[] = TRANG_THAI_DE_XUAT_MUA_HANG.map((id) => ({
      id,
...
export function useDeXuatMuaHangStats(list: DeXuatMuaHang[]) {
  return useMemo(() => computeDeXuatMuaHangStats(list), [list]);
}
