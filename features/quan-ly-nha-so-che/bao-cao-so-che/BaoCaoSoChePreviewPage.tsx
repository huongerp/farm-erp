/**
 * Trang preview in báo cáo sơ chế — toolbar Đóng + Tải xuống + In (A4 dọc).
 * Route: /quan-ly-nha-so-che/bao-cao-so-che/preview/:id
 */
...
import {
  ...
  exportBaoCaoSoCheToXLSX,
  type BaoCaoSoCheExportFormat,
} from './utils/export-bao-cao-so-che';
import BaoCaoSoChePreviewContent from './components/BaoCaoSoChePreviewContent';
import { formatDateShort } from '../../../lib/utils';

const EXPORT_OPTIONS: { format: BaoCaoSoCheExportFormat; label: string; icon: React.ReactNode }[] = [
...
const BaoCaoSoChePreviewPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    document.title = `${t('baoCaoSoChe.preview.title')} - ${formatDateShort(data.ngay)}`;
    return () => {
      document.title = prev;
    };
  }, [data, t]);
  ...
  const handleClose = useCallback(() => {
    if (window.history.length > 1) {
      ...
      navigate('/quan-ly-nha-so-che/bao-cao-so-che', { replace: true });
    }
  }, [navigate]);
  ...
  const handleExport = useCallback(
    async (format: BaoCaoSoCheExportFormat) => {
      if (!data) return;
      setDownloadOpen(false);
      setExporting(true);
      try {
  ...
  return (
    <div
      className="bao-cao-so-che-preview-backdrop fixed inset-0 z-[70] flex flex-col bg-muted/90"
      role="main"
      aria-label={t('baoCaoSoChe.preview.title')}
    >
      <div className="bao-cao-so-che-preview-toolbar flex items-center justify-between gap-3 px-4 py-3 bg-card border-b border-border shadow-sm shrink-0 print:hidden">
        <button
          type="button"
          onClick={handleClose}
        ...
        <div className="flex items-center gap-2">
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <Download size={16} />
              {t('baoCaoSoChe.preview.download')}
          ...
          >
            <Printer size={16} />
            {t('baoCaoSoChe.preview.print')}
          </button>
        </div>
      </div>

      <div className="bao-cao-so-che-preview-body flex-1 overflow-y-auto p-4 md:p-6 flex flex-col items-center gap-4">
        <div
