...
export function getDateRangeFromPreset(
  ...
  switch (preset) {
    case 'this_week': {
      ...
      label = `${i18n.t('deXuatChiPhi.stats.dateRange.week')} ${start.getDate()}/${start.getMonth() + 1} – ${end.getDate()}/${end.getMonth() + 1}/${end.getFullYear()}`;
      break;
    }
    case 'last_week': {
      const day = now.getDay();
    ...
    case 'this_month': {
      ...
      label = `${i18n.t('deXuatChiPhi.stats.dateRange.month')} ${now.getMonth() + 1}/${now.getFullYear()}`;
      break;
    }
    case 'last_month': {
      ...
      label = `${i18n.t('deXuatChiPhi.stats.dateRange.month')} ${last.getMonth() + 1}/${last.getFullYear()}`;
      break;
    }
    case 'this_quarter': {
      ...
      label = `${i18n.t('deXuatChiPhi.stats.dateRange.quarter')} ${q}/${now.getFullYear()}`;
      break;
    }
    case 'last_quarter': {
      ...
      label = `${i18n.t('deXuatChiPhi.stats.dateRange.quarter')} ${q}/${lastQ.getFullYear()}`;
      break;
    }
    case 'this_year': {
      start = startOfYear(now);
      end = endOfYear(now);
      label = `${i18n.t('deXuatChiPhi.stats.dateRange.year')} ${now.getFullYear()}`;
      break;
    }
    case 'last_year': {
      ...
      label = `${i18n.t('deXuatChiPhi.stats.dateRange.year')} ${lastYear}`;
      break;
    }
    case 'custom':
    default: {
      ...
      if (customStart && customEnd) {
        ...
        label = i18n.t('deXuatChiPhi.stats.dateRange.custom');
      }
      break;
    }
  }
...
/** Format Date to YYYY-MM-DD for comparison with item.ngay */
