    ...
    <div className="absolute right-0 top-full mt-1.5 w-52 bg-card rounded-xl shadow-xl border border-border z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
      <div className="px-3 py-2 border-b border-border">
        <p className="text-2xs font-semibold text-muted-foreground uppercase tracking-wider">
          {t('deXuatChiPhi.stats.selectFormat')}
        </p>
      </div>
      <div className="p-1.5">
        <button
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="text-2xs text-muted-foreground">{t('deXuatChiPhi.stats.dataAndMeta')}</p>
          </div>
        </button>
        <button
          type="button"
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="text-2xs text-muted-foreground">{t('deXuatChiPhi.stats.printReport')}</p>
          </div>
        </button>
      </div>
    </div>
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        {onPrint && (
          <Tooltip content={t('deXuatChiPhi.stats.printReport')} placement="bottom">
            <Button
              variant="outline"
              size="sm"
              onClick={onPrint}
    ...
    <div className="flex items-center gap-2">
      <div className="relative" ref={ref}>
        ...
        >
          ...
          <span>{isExporting ? t('deXuatChiPhi.stats.exporting') : t('deXuatChiPhi.stats.exportReport')}</span>
          {!isExporting && (
