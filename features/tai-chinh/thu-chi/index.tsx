...
const ThuChiPage: React.FC = () => {
  ...
  const tabs = useMemo(
    () => [
      { id: 'list', label: t('thuChi.tabs.list'), icon: List },
      { id: 'stats', label: t('thuChi.tabs.stats'), icon: BarChart3 },
    ],
    [t]
  );
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-3.75rem)] md:h-[calc(100dvh-4.5rem)] relative">
      ...
      <div className="flex-1 min-h-0 flex flex-col mt-1.5">
        ...
        {activeTab === 'stats' && <ThongKeTab onBack={() => handleTabChange('list')} />}
      </div>
    </div>
  );
};

export default ThuChiPage;
