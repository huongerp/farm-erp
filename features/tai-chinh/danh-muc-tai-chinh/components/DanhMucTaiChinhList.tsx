...
import { getLevelBadgeStyleDefault, getNameStyleDefault } from '../../../../lib/tree-utils';
import type { ColumnConfig } from '../../../../store/createGenericStore';
import { getColumnCellStyle } from '../../../../store/createGenericStore';
import Tooltip from '../../../../components/ui/Tooltip';
import type { QuyenForListMap } from '../hooks/use-hang-muc-quyen';
...
interface Props {
  ...
  onView?: (item: HangMucTaiChinh) => void;
  /** Map quyền theo hạng mục (để hiển thị cột Quyền xem, Quyền quản lý). */
  quyenMap?: QuyenForListMap | null;
}

const treeOptions = {
  getId: (d: HangMucTaiChinh) => d.id,
  getParentId: (d: HangMucTaiChinh) => d.id_cha,
  getOrder: (d: HangMucTaiChinh) => d.thu_tu,
  includeOrphans: true as const,
};

/** Map column id (snake_case) -> locale key (camelCase) để việt hóa tiêu đề cột theo ngôn ngữ hiện tại */
const COLUMN_LABEL_KEYS: Record<string, string> = {
  ...
  mo_ta: 'moTa',
  quyen_xem: 'quyenXem',
  quyen_quan_ly: 'quyenQuanLy',
  trang_thai: 'trangThai',
  tg_cap_nhat: 'tgCapNhat',
};
...
const DanhMucTaiChinhList: React.FC<Props> = ({
  data,
  columns,
  selectedIds,
  onToggleSelection,
  ...
  const getQuyenNames = (idHangMuc: string, loai: 'quan_ly' | 'de_xuat'): string[] => {
    if (!quyenMap) return [];
    const ids = quyenMap.byHangMuc[idHangMuc]?.[loai] ?? [];
    return ids.map((id) => quyenMap.chucVuNames[id] ?? id);
  };
  ...
  if (isLoading) {
    return (
      <ListPageSkeleton
        loadingText={t('danhMucTaiChinh.loading')}
  ...
  if (data.length === 0) {
    return (
      <div className="flex-1 min-h-0 flex items-center justify-center p-4">
        <EmptyState
          title={t('danhMucTaiChinh.empty')}
          description={t('danhMucTaiChinh.emptyHint')}
  ...
  const renderCell = (item: HangMucTaiChinh, col: ColumnConfig) => {
    ...
    switch (col.id) {
      ...
      case 'loai':
        return (
          <td key={col.id} className="px-6 py-3.5" style={getColumnCellStyle(col)}>
            ...
            >
              {item.loai === 'thu' ? t('danhMucTaiChinh.loaiThu') : t('danhMucTaiChinh.loaiChi')}
            </span>
          </td>
        );
      case 'ten_cha':
      ...
      case 'quyen_xem': {
        const names = getQuyenNames(item.id, 'de_xuat');
        return (
          <td key={col.id} className="px-6 py-3.5 min-w-0" style={getColumnCellStyle(col)}>
            <span className="text-xs text-muted-foreground line-clamp-2" title={names.join(', ')}>
      ...
      case 'quyen_quan_ly': {
        const names = getQuyenNames(item.id, 'quan_ly');
        return (
          <td key={col.id} className="px-6 py-3.5 min-w-0" style={getColumnCellStyle(col)}>
            <span className="text-xs text-muted-foreground line-clamp-2" title={names.join(', ')}>
              {names.length ? names.join(', ') : '—'}
    ...
    <div className="flex flex-col h-full min-h-0 bg-card overflow-hidden">
              ...
              <thead className="sticky top-0 z-[2]">
                <tr className="bg-muted border-b border-border">
                  ...
                  {visibleColumns.map((col) => (
                    ...
                    >
                      {t(`danhMucTaiChinh.columns.${COLUMN_LABEL_KEYS[col.id] ?? col.id}`)}
                    </th>
                  ))}
                  <th className="sticky right-0 z-[3] min-w-[142px] px-3 py-2 border-b border-l border-border text-center font-semibold text-foreground/80 text-xs shadow-[inset 8px 0 8px -8px rgba(0,0,0,0.12)] bg-[hsl(var(--muted))]">
                    {actionsLabel}
                  </th>
                </tr>
              </thead>
                ...
                {paginatedData.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={cn(
                          'sticky right-0 z-[1] min-w-[142px] px-2 py-3.5 border-l border-border/50 text-center transition-colors shadow-[inset 8px 0 8px -8px rgba(0,0,0,0.08)] group-hover:bg-[hsl(var(--muted))]',
                          isSelected && '!bg-[hsl(var(--primary)/0.12)]',
                          isRoot && !isSelected && 'bg-[hsl(var(--muted))]',
                          !isRoot && !isSelected && 'bg-[hsl(var(--card))]'
                        )}
                      ...
                      >
                        <div className="flex items-center justify-end gap-0 flex-nowrap">
                          {isParent && onAddChild && (
                            <Tooltip content={t('danhMucTaiChinh.detail.addChild')} placement="top">
                              <button
                                type="button"
                          ...
                          {isChild && (
                            <>
                              <Tooltip content={t('danhMucTaiChinh.columns.quyenXem')} placement="top">
                                <button
                                  type="button"
                                  onClick={() => onOpenQuyenDeXuat(item)}
                                  className="p-1.5 text-cyan-600 hover:bg-cyan-50 dark:hover:bg-cyan-950/30 rounded-md transition-all active:scale-95 shrink-0"
                                >
                                  <FileEdit size={15} />
                                </button>
                              </Tooltip>
                              <Tooltip content={t('danhMucTaiChinh.quyenQuanLy')} placement="top">
                                <button
                                  type="button"
                                  onClick={() => onOpenQuyenQuanLy(item)}
                                  className="p-1.5 text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-950/30 rounded-md transition-all active:scale-95 shrink-0"
                                >
                                  <Shield size={15} />
                                </button>
                              </Tooltip>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isChild && quyenMap && (
                        <div className="mt-1.5 space-y-0.5 text-[11px] text-muted-foreground">
                          {(() => {
                            const xem = getQuyenNames(item.id, 'de_xuat');
                            const qly = getQuyenNames(item.id, 'quan_ly');
                            return (
                              <>
                                {xem.length > 0 && (
                                  <div><span className="font-medium">{t('danhMucTaiChinh.columns.quyenXem')}:</span> {xem.join(', ')}</div>
                                )}
                                {qly.length > 0 && (
                                  <div><span className="font-medium">{t('danhMucTaiChinh.columns.quyenQuanLy')}:</span> {qly.join(', ')}</div>
                                )}
                              </>
                            );
                          })()}
                      ...
                      {isChild && (
                        <>
                          <button
                            type="button"
                            onClick={() => onOpenQuyenDeXuat(item)}
                            className="p-2 text-cyan-600 hover:bg-cyan-50 rounded-lg text-xs inline-flex items-center gap-1"
                          >
                            <FileEdit size={14} />
                            {t('danhMucTaiChinh.quyenDeXuat')}
                          </button>
                          <button
                            type="button"
                            onClick={() => onOpenQuyenQuanLy(item)}
                            className="p-2 text-violet-600 hover:bg-violet-50 rounded-lg text-xs inline-flex items-center gap-1"
                          >
                            <Shield size={14} />
                            {t('danhMucTaiChinh.quyenQuanLy')}
                          </button>
                        </>
                      )}
                    </div>
      ...
      <div className="shrink-0 border-t border-border bg-muted/30">
        <TablePaginationFooter
          ...
          recordsLabel={t('danhMucTaiChinh.footerRecords')}
        />
      </div>
    </div>
  );
...
export default DanhMucTaiChinhList;
