...
export type HopDongBaoCaoExportFormat = 'excel' | 'pdf';

export interface HopDongExportDropdownProps {
  onExport: (format: HopDongBaoCaoExportFormat) => Promise<void>;
  disabled?: boolean;
  compact?: boolean;
}

const HopDongExportDropdown: React.FC<HopDongExportDropdownProps> = ({
  onExport,
  disabled = false,
  compact = false,
}) => {
  ...
  const handleSelect = async (format: HopDongBaoCaoExportFormat) => {
    setIsExporting(true);
    setOpen(false);
    try {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span>
          {isExporting ? t('hopDong.export.toolbar.exporting') : t('hopDong.export.toolbar.exportReport')}
        </span>
        {!isExporting && (
        ...
        <div className="absolute right-0 top-full mt-1.5 w-52 bg-card rounded-xl shadow-xl border border-border z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
          <div className="px-3 py-2 border-b border-border">
            <p className="text-2xs font-semibold text-muted-foreground uppercase tracking-wider">
              {t('hopDong.export.toolbar.selectFormat')}
            </p>
          </div>
          <div className="p-1.5">
            <button
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-2xs text-muted-foreground">{t('hopDong.export.toolbar.exportExcelDesc')}</p>
              </div>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-2xs text-muted-foreground">{t('hopDong.export.toolbar.exportPdfDesc')}</p>
              </div>
            </button>
          </div>
        </div>
...
export default HopDongExportDropdown;
