...
interface Props {
  ...
  onRowClick?: (item: TienDoMuaHang) => void;
}

const TienDoMuaHangTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onRowClick }) => {
  ...
  const renderCell = (colId: string, item: TienDoMuaHang) => {
    switch (colId) {
      case 'ma':
        return (
  ...
  const renderMobileCard = (item: TienDoMuaHang, isSelected: boolean) => (
    <div
      role="button"
      tabIndex={0}
...
export default TienDoMuaHangTable;
