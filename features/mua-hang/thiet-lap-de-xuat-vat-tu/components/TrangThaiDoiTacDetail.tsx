...
interface Props {
  ...
  onClose: () => void;
  onEdit: (item: TrangThaiDoiTac) => void;
  onDelete?: (id: string) => void;
}

const TrangThaiDoiTacDetail: React.FC<Props> = ({ data, onClose, onEdit, onDelete }) => {
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <DetailSection title={t('thietLapDeXuatVatTu.doiTac.form.basicInfo')} icon={<Tag size={14} />} variant="primary">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <DetailField label={t('thietLapDeXuatVatTu.doiTac.form.ma')} value={data.ma} />
            <DetailField label={t('thietLapDeXuatVatTu.doiTac.form.ten')} value={data.ten} />
            <DetailField label={t('thietLapDeXuatVatTu.doiTac.form.thuTu')} value={String(data.thu_tu)} />
            {data.mau ? (
              <div className="flex items-center gap-2">
                <DetailField label={t('thietLapDeXuatVatTu.doiTac.store.mauCol')} value={data.mau} />
                <span
            ...
            {data.ghi_chu ? (
              <div className="col-span-1 sm:col-span-2">
                <DetailField label={t('thietLapDeXuatVatTu.doiTac.form.ghiChu')} value={data.ghi_chu} />
              </div>
            ) : null}
            <DetailField
...
export default TrangThaiDoiTacDetail;
