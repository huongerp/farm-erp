...
const DonDatHangList: React.FC<Props> = ({
  ...
  const renderCell = (item: DonDatHang, col: ColumnConfig) => {
    switch (col.id) {
      case 'so_po':
        return (
      ...
      case 'ngay_dat':
        return (
          <td key={col.id} className="px-4 py-3" style={getColumnCellStyle(col)}>
            <span className="text-sm text-muted-foreground">{item.ngay_dat}</span>
          </td>
        );
      case 'ngay_giao_dk':
        return (
          <td key={col.id} className="px-4 py-3" style={getColumnCellStyle(col)}>
            <span className="text-sm text-muted-foreground">{item.ngay_giao_dk}</span>
          </td>
        );
      case 'ten_nha_cung_cap':
        return (
          <td key={col.id} className="px-4 py-3 min-w-0" style={getColumnCellStyle(col)}>
            <span className="text-sm text-muted-foreground">{item.ten_nha_cung_cap ?? '—'}</span>
          </td>
        );
      case 'ten_kho_nhan':
        return (
          <td key={col.id} className="px-4 py-3 min-w-0" style={getColumnCellStyle(col)}>
            <span className="text-sm text-muted-foreground">{item.ten_kho_nhan ?? '—'}</span>
          </td>
        );
      case 'ten_nguoi_dat':
        return (
          <td key={col.id} className="px-4 py-3 min-w-0" style={getColumnCellStyle(col)}>
            <span className="text-sm text-muted-foreground">{item.ten_nguoi_dat ?? '—'}</span>
          </td>
        );
      case 'trang_thai':
        return (
  ...
  if (data.length === 0) {
    return (
      <div className="flex-1 min-h-0 flex items-center justify-center p-4">
        <EmptyState
          title={t('donDatHang.empty')}
          description={t('donDatHang.emptyHint')}
