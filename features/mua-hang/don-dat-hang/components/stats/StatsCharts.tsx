...
const StatsCharts: React.FC<Props> = ({ byTrangThai, bySupplier, byBuyer, byMonth, byDanhMucCap1, byDanhMucCap2, byPhanLoai }) => {
  ...
  const statusPieData = byTrangThai
    .filter((d) => d.count > 0)
    .map((d) => ({ name: t(`donDatHang.${d.ten}`), value: d.count }));
    ...
    <div className="bg-card rounded-xl border border-border p-3.5">
      ...
      <ResponsiveContainer width="100%" height={STATS_CHART_HEIGHT}>
        ...
        >
          ...
          <Bar dataKey="value" radius={layout === 'vertical' ? [0, 6, 6, 0] : [6, 6, 0, 0]} name={t('donDatHang.stats.countCol')}>
            {data.map((_, i) => (
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {hasByMonth && renderBarChart(byMonth, 'donDatHang.stats.byMonth', <TrendingUp size={14} className="text-primary" />, 'horizontal')}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
