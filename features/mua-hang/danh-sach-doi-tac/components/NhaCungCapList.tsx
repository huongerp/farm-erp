...
interface Props {
  ...
  onView?: (item: NhaCungCap) => void;
}

const COL_LABEL_KEYS: Record<string, string> = {
  thu_tu: 'nhaCungCapMuaHang.store.orderCol',
  ma_ncc: 'nhaCungCapMuaHang.store.codeCol',
  ten_ncc: 'nhaCungCapMuaHang.store.nameCol',
  ten_nhom: 'nhaCungCapMuaHang.store.groupCol',
  dia_chi: 'nhaCungCapMuaHang.store.addressCol',
  dien_thoai: 'nhaCungCapMuaHang.store.phoneCol',
  email: 'nhaCungCapMuaHang.store.emailCol',
  tags: 'nhaCungCapMuaHang.store.tagsCol',
  trang_thai: 'nhaCungCapMuaHang.store.statusCol',
  tg_cap_nhat: 'nhaCungCapMuaHang.store.updatedCol',
};

const NhaCungCapList: React.FC<Props> = ({
  data,
  columns,
  selectedIds,
  onToggleSelection,
  ...
  const renderCell = (item: NhaCungCap, col: ColumnConfig) => {
    switch (col.id) {
      case 'thu_tu':
        return (
      ...
      case 'dia_chi':
        return (
          <td key={col.id} className="px-4 py-3 min-w-0" style={getColumnCellStyle(col)}>
            <span className="text-sm text-muted-foreground">{item.dia_chi ?? '—'}</span>
          </td>
        );
      case 'dien_thoai':
        return (
  ...
  if (isLoading) {
    return (
      <ListPageSkeleton
        loadingText={t('nhaCungCapMuaHang.loading')}
  ...
  if (data.length === 0) {
    return (
      <div className="flex-1 min-h-0 flex items-center justify-center p-4">
        <EmptyState
          title={t('nhaCungCapMuaHang.empty')}
          description={t('nhaCungCapMuaHang.emptyHint')}
    ...
    <div className="flex flex-col h-full min-h-0 bg-card overflow-hidden">
            ...
            <thead className="sticky top-0 z-10 bg-muted/95 border-b border-border">
              <tr>
                ...
                {visibleColumns.map((col) => (
                  ...
                  >
                    {COL_LABEL_KEYS[col.id] ? t(COL_LABEL_KEYS[col.id]) : col.label}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-border [&>tr:last-child>td]:border-b [&>tr:last-child>td]:border-border">
              {paginatedData.map((item) => (
                <tr
                  ...
                  onKeyDown={
                    onView ? (e) => e.key === 'Enter' && onView(item) : undefined
                  }
      ...
      <div className="shrink-0 border-t border-border bg-muted/30">
        <TablePaginationFooter
          ...
          recordsLabel={t('nhaCungCapMuaHang.footerRecords')}
        />
      </div>
    </div>
  );
