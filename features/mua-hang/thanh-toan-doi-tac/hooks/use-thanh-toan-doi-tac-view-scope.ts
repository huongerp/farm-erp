 ...
 * - thu_tu chức vụ ≠ 1 → chỉ xem theo các chi nhánh được phân (chi_nhanh_ids).
 * Thanh toán đối tác: lọc theo id_don_vi (đơn vị = chi nhánh trong code).
 */
...
export interface ThanhToanDoiTacViewScope {
  viewAll: boolean;
...
export function useThanhToanDoiTacViewScope(): ThanhToanDoiTacViewScope {
  ...
  return useMemo(() => {
    const defaultScope: ThanhToanDoiTacViewScope = {
      viewAll: true,
      viewByBranch: false,
      allowedBranchIds: [],
