...
const FarmTienDoMuaHangToolbar: React.FC<Props> = ({ items = [], onAdd, onDeleteMany, onStatusChangeMany, canCreate = true, canDelete = true }) => {
  const { t } = useTranslation();
  const { searchInput, setSearchInput } = useGenericToolbarSearch(useFarmTienDoMuaHangStore);
  const filters = useFarmTienDoMuaHangStore((s) => s.filters);
  const setFilter = useFarmTienDoMuaHangStore((s) => s.setFilter);
  const columns = useFarmTienDoMuaHangStore((s) => s.columns);
...
export default FarmTienDoMuaHangToolbar;
