...
interface Props {
  data: FarmTienDoMuaHang;
  onClose: () => void;
  onEdit?: (item: FarmTienDoMuaHang) => void;
  onDelete?: (id: string) => void;
}

const FarmTienDoMuaHangDetail: React.FC<Props> = ({ data, onClose, onEdit, onDelete }) => {
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <DetailSection title={t('thietLapDeXuatMuaHang.tienDoMuaHang.form.basicInfo')} icon={<Package size={14} />} variant="primary">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <DetailField label={t('thietLapDeXuatMuaHang.tienDoMuaHang.form.ma')} value={data.ma} />
            <DetailField label={t('thietLapDeXuatMuaHang.tienDoMuaHang.form.ten')} value={data.ten} />
            <DetailField label={t('thietLapDeXuatMuaHang.tienDoMuaHang.form.thuTu')} value={String(data.thu_tu)} />
            <div className="flex items-center gap-3">
              <DetailField label={t('thietLapDeXuatMuaHang.tienDoMuaHang.store.mauCol')} value={data.mau || TRANG_THAI_MAU_DEFAULT} />
              <span
            ...
            {data.ghi_chu ? (
              <div className="col-span-1 sm:col-span-2">
                <DetailField label={t('thietLapDeXuatMuaHang.tienDoMuaHang.form.ghiChu')} value={data.ghi_chu} />
              </div>
            ) : null}
            <DetailField
            ...
            />
            <DetailField label={t('thietLapDeXuatMuaHang.tienDoMuaHang.store.updatedCol')} value={formatDateTimeShort(data.tg_cap_nhat)} />
          </div>
        </DetailSection>
      </div>
    </GenericDrawer>
...
export default FarmTienDoMuaHangDetail;
