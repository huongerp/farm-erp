...
interface Props {
  ...
  onRowClick?: (item: FarmTienDoMuaHang) => void;
}

const FarmTienDoMuaHangTable: React.FC<Props> = ({ data, isLoading, onEdit, onDelete, onRowClick }) => {
  ...
  const renderCell = (colId: string, item: FarmTienDoMuaHang) => {
    switch (colId) {
      case 'ma':
        return (
  ...
  const renderMobileCard = (item: FarmTienDoMuaHang, isSelected: boolean) => (
    <div
      role="button"
      tabIndex={0}
...
export default FarmTienDoMuaHangTable;
