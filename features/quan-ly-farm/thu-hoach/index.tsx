...
const VALID_TABS = ['nhap-lieu', 'stats'] as const;
type TabId = (typeof VALID_TABS)[number];

const ThuHoachPage: React.FC = () => {
  const { t } = useTranslation();
  const [searchParams, setSearchParams] = useSearchParams();
  const [activeTab, setActiveTab] = useState<TabId>('nhap-lieu');
  ...
  const tabs = useMemo(
    () => [
      { id: 'nhap-lieu', label: t('thuHoach.tabs.nhapLieu'), icon: List },
      { id: 'stats', label: t('thuHoach.tabs.thongKe'), icon: BarChart3 },
    ],
    [t]
  );
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-3.75rem)] md:h-[calc(100dvh-4.5rem)] relative">
      ...
      <div className="flex-1 min-h-0 flex flex-col mt-1.5">
        {activeTab === 'nhap-lieu' && <DanhSachTab />}
        {activeTab === 'stats' && (
