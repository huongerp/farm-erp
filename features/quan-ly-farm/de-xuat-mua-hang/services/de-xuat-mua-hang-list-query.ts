...
import type { DeXuatMuaHangFilters } from '../store/useDeXuatMuaHangStore';
...
export type DeXuatMuaHangListServerQuery = {
  searchTerm: string;
...
export function buildDeXuatMuaHangListServerQuery(params: {
  searchTerm: string;
  filters: DeXuatMuaHangFilters;
  ngayFrom: string;
  ngayTo: string;
  viewScope: EmployeeBranchModuleScope;
  khoList: Kho[];
}): DeXuatMuaHangListServerQuery {
...
export type DeXuatMuaHangChiTietListServerQuery = {
  searchTerm: string;
...
export function buildDeXuatMuaHangChiTietListServerQuery(params: {
  searchTerm: string;
  ...
  viewScope: EmployeeBranchModuleScope;
  khoList: Kho[];
}): DeXuatMuaHangChiTietListServerQuery {
  const { searchTerm, filters, ngayFrom, ngayTo, viewScope, khoList } = params;
  const scope = buildEmployeeBranchScopeForServer(
    viewScope.viewAll,
    viewScope.viewByBranch,
