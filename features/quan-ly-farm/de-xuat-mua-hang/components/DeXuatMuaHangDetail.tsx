...
import { FileText, Calendar, Warehouse, User, UserCheck, Package, CheckCircle, Printer, X, Copy, PackagePlus, Hourglass } from 'lucide-react';
...
export interface DeXuatMuaHangApprovePayload {
  trangThai: TrangThaiDeXuatMuaHang;
  ghiChu?: string;
}
...
interface Props {
  ...
  onCopy?: (item: DeXuatMuaHang) => void;
  onApprove?: (item: DeXuatMuaHang, payload: DeXuatMuaHangApprovePayload) => void;
  onPrint?: (item: DeXuatMuaHang) => void;
  /** Tạo phiếu kho nhập phân thuốc từ phiếu đã duyệt (chỉ truyền khi user có quyền tạo phiếu kho). */
  onCreatePhieuKho?: (item: DeXuatMuaHang) => void;
  canEdit?: boolean;
  canDelete?: boolean;
...
const DeXuatMuaHangDetail: React.FC<Props> = ({
  ...
  const submitApprove = (trangThai: TrangThaiDeXuatMuaHang) => {
  ...
  const toolbarActions: DetailToolbarAction[] = useMemo(
    () => [
      ...(canApprove
        ? [
            {
              label: t('deXuatMuaHang.detail.toolbar.approve'),
      ...
      ...(onCopy
        ? [
            {
              label: t('deXuatMuaHang.detail.toolbar.copy'),
              icon: <Copy size={16} />,
      ...
      ...(canCreatePhieuKho
        ...
        : []),
      {
        label: t('deXuatMuaHang.detail.toolbar.print'),
  ...
  return (
    <>
    <GenericDrawer
      title={t('deXuatMuaHang.detail.title')}
    ...
    >
      <div className="space-y-5">
        <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
              ...
              {soPhieuKhoDaTao && (
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-teal-500/10 text-teal-700 dark:text-teal-300 border border-teal-500/20">
                  {t('deXuatMuaHang.detail.phieuKhoAlreadyCreated', { soPhieu: soPhieuKhoDaTao })}
                </span>
              )}
              {showOverdueBadge && (
        ...
        <DetailSection title={t('deXuatMuaHang.detail.basicInfo')} icon={<FileText size={14} />} variant="primary">
          <DetailFieldGrid>
            <DetailField label={t('deXuatMuaHang.form.code')} value={data.so_phieu} icon={<FileText size={12} />} />
            <DetailField label={t('deXuatMuaHang.form.date')} value={data.ngay} icon={<Calendar size={12} />} />
            <DetailField
        ...
        <DetailSection title={t('deXuatMuaHang.detail.systemInfo')} icon={<Calendar size={14} />} variant="secondary">
          <DetailFieldGrid>
            <DetailField
              label={t('deXuatMuaHang.detail.createdAt')}
              value={formatDateTimeShort(data.tg_tao)}
              icon={<Calendar size={12} />}
            />
            <DetailField
              label={t('deXuatMuaHang.detail.updatedAt')}
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="text-lg font-semibold text-foreground">
                      {t('deXuatMuaHang.detail.approveDialogTitle')}
                    </h3>
              ...
              <div className="px-6 py-5 space-y-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">
                    {t('deXuatMuaHang.detail.approveDialogResult')}
                  </p>
                  <div className="grid grid-cols-3 gap-2">
                    <ApproveOption
                      label={t('deXuatMuaHang.status.approved')}
                    ...
                    />
                    <ApproveOption
                      label={t('deXuatMuaHang.status.waiting')}
                    ...
                    />
                    <ApproveOption
                      label={t('deXuatMuaHang.status.rejected')}
                      icon={<X size={14} />}
                ...
                <Textarea
                  label={t('deXuatMuaHang.detail.approveDialogNote')}
                  placeholder={t('deXuatMuaHang.detail.approveDialogNotePlaceholder')}
                  value={approveGhiChu}
...
export default DeXuatMuaHangDetail;
