...
import { useDeXuatMuaHangChiTietPaged, useDeXuatMuaHangById, useUpdateDeXuatMuaHang } from '../hooks/use-de-xuat-mua-hang';
import { useDeXuatMuaHangViewScope } from '../hooks/use-de-xuat-mua-hang-view-scope';
import {
  buildDeXuatMuaHangChiTietListServerQuery,
  fetchAllDeXuatMuaHangChiTietForListQuery,
...
const ChiTietTab: React.FC = () => {
  ...
  const [viewingRow, setViewingRow] = useState<DeXuatMuaHangChiTietRow | null>(null);
  ...
  const { data: phieuForEdit } = useDeXuatMuaHangById(editingRow?.id_de_xuat_mua_hang);
  const updateMutation = useUpdateDeXuatMuaHang();
  ...
  const handleSaveEdit = useCallback(
    (payload: ChiTietRowEditPayload) => {
      ...
      const data: DeXuatMuaHangFormValues = {
  ...
  const handleChuyenTienDoConfirm = useCallback(
    async (result: ChuyenTienDoResult) => {
      ...
      const byPhieu = new Map<string, DeXuatMuaHangChiTietRow[]>();
      selectedRows.forEach((r) => {
        const list = byPhieu.get(r.id_de_xuat_mua_hang) ?? [];
        list.push(r);
        byPhieu.set(r.id_de_xuat_mua_hang, list);
      });
      const traoDoiLine =
      ...
      try {
        ...
        toast.success(t('deXuatMuaHang.chiTietTab.chuyenTienDoSuccess'));
  ...
  const handleDelete = useCallback(
    (row: DeXuatMuaHangChiTietRow) => {
      confirm({
        ...
        message: t('deXuatMuaHang.chiTietTab.deleteLineConfirm'),
        variant: 'danger',
  ...
  const sortedRows = useMemo(() => {
    ...
    const key = sort.column as keyof DeXuatMuaHangChiTietRow;
  ...
  const phieuById = useMemo(() => new Map<string, DeXuatMuaHang>(), []);
  ...
  const exportMapFn = useCallback(
    (row: DeXuatMuaHangChiTietRow) =>
      mapChiTietRowToExport(row, phieuById.get(row.id_de_xuat_mua_hang), employees, khoById),
  ...
  const { exportData, paginatedData: paginatedExportData, selectedData: selectedExportData } = useExportData({
    ...
    pagination,
    selectedIds,
    keyExtractor: (r: DeXuatMuaHangChiTietRow) => r.id,
  });
  ...
  const renderStatusBadge = useCallback((status: string | null) => {
    ...
    return (
      ...
      >
        {t(`deXuatMuaHang.status.${trangThaiToI18nKey(status)}`, { defaultValue: status })}
      </span>
    );
  }, [t]);
  ...
  const renderCell = useCallback(
    (colId: string, item: DeXuatMuaHangChiTietRow) => {
      switch (colId) {
        case 'so_phieu':
          return (
        ...
        case 'ngay':
        case 'ngay_can':
          return <span className="text-sm text-muted-foreground whitespace-nowrap">{item[colId as keyof DeXuatMuaHangChiTietRow] ?? '—'}</span>;
        case 'ten_noi_de_xuat':
        case 'ten_nguoi_de_xuat':
        case 'ten_nguoi_duyet':
          return (
            <span className="block truncate whitespace-nowrap text-sm" title={String(item[colId as keyof DeXuatMuaHangChiTietRow] ?? '')}>
              {String(item[colId as keyof DeXuatMuaHangChiTietRow] ?? '—')}
            </span>
          );
  ...
  const renderMobileCard = useCallback(
    (item: DeXuatMuaHangChiTietRow, isSelected: boolean) => (
      <div
  ...
  const bulkActions = hasSelection ? (
    <>
      ...
      >
        ...
        <span className="text-xs font-medium">{t('deXuatMuaHang.chiTietTab.tienDoAction')}</span>
      </Button>
    </>
  ) : null;
  ...
  return (
    <>
      <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-border bg-card shadow-sm overflow-hidden">
        ...
        />
        <GenericTable<DeXuatMuaHangChiTietRow>
          data={sortedRows}
          columns={columns}
          ...
          emptyTitle={t('deXuatMuaHang.chiTietTab.emptyTitle')}
          emptyDescription={t('deXuatMuaHang.chiTietTab.emptyDescription')}
        />
      </div>
