...
const DeXuatMuaHangPreviewContent: React.FC<Props> = ({ phieu }) => {
  ...
  return (
    <div className="de-xuat-mua-hang-preview-content de-xuat-mua-hang-preview bg-white text-gray-900 font-sans text-[10pt] pt-[15mm] pr-[15mm] pb-[15mm] pl-[20mm] print:p-0 min-h-full flex flex-col">
      ...
      <div className="text-[10pt] space-y-1.5 mb-4">
        <p>
          <span className="font-semibold text-gray-600">{t('deXuatMuaHang.preview.donVi')}: </span>
          <span className="text-gray-900">{phieu.ten_noi_de_xuat ?? '—'}</span>
        </p>
        <p>
          <span className="font-semibold text-gray-600">{t('deXuatMuaHang.preview.nguoiTao')}: </span>
          <span className="text-gray-900">{phieu.ten_nguoi_de_xuat ?? '—'}</span>
          <span className="text-gray-600"> . {t('deXuatMuaHang.preview.boPhan')}: —</span>
        </p>
        <p>
          <span className="font-semibold text-gray-600">{t('deXuatMuaHang.preview.ngayLap')} </span>
          <span className="text-gray-900">{formatDate(phieu.ngay)}</span>
          <span className="text-gray-600"> ({t('deXuatMuaHang.form.requiredDate')}: {formatDate(phieu.ngay_can)})</span>
        </p>
        <p>
          <span className="font-semibold text-gray-600">{t('deXuatMuaHang.form.notes')} </span>
      ...
      <h2 className="text-[11pt] font-semibold mt-2 mb-2 text-gray-900">
        {t('deXuatMuaHang.preview.danhSachChiTiet')}
      </h2>
      {chiTiet.length > 0 ? (
        ...
        <p className="text-[10pt] text-gray-500 italic">{t('deXuatMuaHang.form.noItems')}</p>
      )}

      {/* Bốn ô chữ ký */}
      <div className="grid grid-cols-4 gap-4 mt-8 pt-4 border-t border-gray-300">
        <div className="text-center">
          <p className="text-[10pt] font-semibold text-gray-800 mb-0.5">{t('deXuatMuaHang.preview.signCreator')}</p>
          <p className="text-[8pt] text-gray-500">{t('deXuatMuaHang.preview.signHint')}</p>
        </div>
        <div className="text-center">
          <p className="text-[10pt] font-semibold text-gray-800 mb-0.5">{t('deXuatMuaHang.preview.signChecker')}</p>
          <p className="text-[8pt] text-gray-500">{t('deXuatMuaHang.preview.signHint')}</p>
        </div>
        <div className="text-center">
          <p className="text-[10pt] font-semibold text-gray-800 mb-0.5">{t('deXuatMuaHang.preview.signRelated')}</p>
          <p className="text-[8pt] text-gray-500">{t('deXuatMuaHang.preview.signHint')}</p>
        </div>
        <div className="text-center">
          <p className="text-[10pt] font-semibold text-gray-800 mb-0.5">{t('deXuatMuaHang.preview.signApprover')}</p>
          <p className="text-[8pt] text-gray-500">{t('deXuatMuaHang.preview.signHint')}</p>
        </div>
      </div>
      ...
      <footer className="mt-auto pt-4 border-t border-gray-200 print:mt-8">
        <p className="text-[7pt] text-gray-500 text-left">
          {t('deXuatMuaHang.preview.printedAt')} {printedAt}
        </p>
      </footer>
    </div>
  );
...
export default DeXuatMuaHangPreviewContent;
