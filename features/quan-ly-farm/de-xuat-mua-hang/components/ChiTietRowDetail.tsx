...
import type { DeXuatMuaHangChiTietRow } from '../core/types';
...
const ChiTietRowDetail: React.FC<Props> = ({ data, onClose, onEdit, onDelete, onChuyenTienDo }) => {
  ...
  const detailToolbarActions: DetailToolbarAction[] = useMemo(
    () =>
      onChuyenTienDo
        ? [
            {
              label: t('deXuatMuaHang.chiTietTab.tienDoAction'),
  ...
  const statusLabel = data.trang_thai_phieu
    ? t(`deXuatMuaHang.status.${trangThaiToI18nKey(data.trang_thai_phieu)}`, { defaultValue: data.trang_thai_phieu })
    : '—';
  ...
  return (
    <GenericDrawer
      title={t('deXuatMuaHang.chiTietTab.detailTitle')}
    ...
    >
      <div className="space-y-5">
        ...
        <DetailSection
          title={t('deXuatMuaHang.chiTietTab.sectionPhieu')}
        ...
        >
          <DetailFieldGrid>
            ...
            />
            <DetailField
              label={t('deXuatMuaHang.store.statusCol')}
        ...
        <DetailSection
          title={t('deXuatMuaHang.chiTietTab.sectionHangHoa')}
          icon={<Package size={14} />}
          variant="primary"
        >
          <DetailFieldGrid>
            <DetailField
              label={t('deXuatMuaHang.form.itemCode')}
              value={data.ma_hang ?? '—'}
              icon={<Package size={12} />}
            />
            <DetailField
              label={t('deXuatMuaHang.form.itemName')}
              value={data.ten_hang ?? '—'}
              icon={<Package size={12} />}
            ...
            />
            <DetailField
              label={t('deXuatMuaHang.form.unit')}
            ...
            />
            <DetailField
              label={t('deXuatMuaHang.store.nguoiDuyetCol')}
              value={data.ten_nguoi_duyet ?? '—'}
              icon={<UserCheck size={12} />}
            />
            <DetailField
              label={t('deXuatMuaHang.form.specs')}
              value={data.thong_so ?? '—'}
              icon={<FileText size={12} />}
              className="col-span-1 sm:col-span-2"
            />
        ...
        <DetailSection
          title={t('deXuatMuaHang.chiTietTab.traoDoiGhiChuSection')}
          icon={<FileText size={14} />}
          variant="primary"
        >
          <DetailFieldGrid>
            ...
            />
            <DetailField
              label={t('deXuatMuaHang.form.note')}
              value={data.ghi_chu ?? '—'}
              icon={<FileText size={12} />}
              className="col-span-1 sm:col-span-2"
            />
