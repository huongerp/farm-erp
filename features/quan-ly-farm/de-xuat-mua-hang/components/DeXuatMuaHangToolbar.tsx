...
const DeXuatMuaHangToolbar: React.FC<Props> = ({
  data,
  khoList,
  employees,
  currentUserId,
  ...
  const searchTerm = useDeXuatMuaHangStore((s) => s.searchTerm);
  const commitSearchTerm = useDeXuatMuaHangStore((s) => s.commitSearchTerm);
  const filters = useDeXuatMuaHangStore((s) => s.filters);
  const setFilter = useDeXuatMuaHangStore((s) => s.setFilter);
  const clearSelection = useDeXuatMuaHangStore((s) => s.clearSelection);
  const columns = useDeXuatMuaHangStore((s) => s.columns);
  ...
  const statusOptions = useMemo(
    () => [
      {
        label: t('deXuatMuaHang.status.pending'),
        value: 'Pending',
...
export default DeXuatMuaHangToolbar;
