...
import type { DeXuatMuaHangStatsByTrangThai } from './useDeXuatMuaHangStats';
import type { StatsChartItem } from './useDeXuatMuaHangStats';
...
const StatsCharts: React.FC<Props> = ({
  ...
  const statusPieData = byTrangThai
    .filter((d) => d.count > 0)
    .map((d) => ({ name: t(`deXuatMuaHang.${d.ten}`), value: d.count }));
    ...
    <div className="bg-card rounded-xl border border-border p-3.5">
      ...
      <ResponsiveContainer width="100%" height={STATS_CHART_HEIGHT}>
        ...
        >
          ...
          <Bar dataKey="value" radius={layout === 'vertical' ? [0, 6, 6, 0] : [6, 6, 0, 0]} name={t('deXuatMuaHang.stats.countCol')}>
            {data.map((_, i) => (
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-card rounded-xl border border-border p-3.5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-xs font-semibold text-foreground">
                {t('deXuatMuaHang.stats.chartByStatus')}
              </h3>
            </div>
            <ResponsiveContainer width="100%" height={STATS_CHART_HEIGHT}>
              <PieChart>
        ...
        {hasByMonth && renderBarChart(byMonth, 'deXuatMuaHang.stats.chartByMonth', <TrendingUp size={14} className="text-primary" />, 'horizontal')}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {hasNoiDeXuat && renderBarChart(byNoiDeXuat.slice(0, 8), 'deXuatMuaHang.stats.chartByPlace', <Warehouse size={14} className="text-primary" />)}
        {hasNguoiDeXuat && renderBarChart(byNguoiDeXuat.slice(0, 8), 'deXuatMuaHang.stats.chartByRequester', <User size={14} className="text-primary" />)}
        {hasNguoiDuyet && renderBarChart(byNguoiDuyet.slice(0, 8), 'deXuatMuaHang.stats.chartByApprover', <UserCheck size={14} className="text-primary" />)}
      </div>
    </div>
  );
};
